 ...
 *
 * LUỒNG DỮ LIỆU:
 *   Thư viện (CONFIG_KPI_LIBRARY) → Template (kpi_list refs) → Snapshot tháng (kpi_snapshot_YYYY-MM)
 *
 * DỮ LIỆU ĐẦU VÀO:
 ...
 *
 * PHÂN QUYỀN (TODO):
 * - Thư viện KPI nên chỉ admin mới sửa được; staff chỉ xem.
 * - Template và snapshot theo tháng: chỉ admin/trưởng phòng mới tạo/xóa.
 *
 * LƯU Ý:
...
function useKpiStore() {
  ...
  const persistKpi  = v => { setKpiList(v); saveKpiList(v); syncToGas('kpi_list', v); };
  const persistNhom = v => { setNhomList(v); saveNhomList(v); syncToGas('nhom_list', v); };
    ...
    <div className="flex gap-1 bg-gray-100 p-1 rounded-lg shrink-0">
      {['ca_nhan', 'phong'].map(c => (
        <button key={c} onClick={() => onChange(c)}
          className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${cap === c ? 'bg-white shadow text-blue-700' : 'text-gray-500 hover:text-gray-700'}`}>
  ...
  return (
    <div className="card border-green-200 bg-green-50 space-y-3">
      ...
      <div className="flex gap-3 items-end flex-wrap">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Mã nhóm (tự động)</label>
          <input className="input bg-gray-100 text-gray-500 text-xs font-mono w-32" value={form.nhom_id} readOnly />
        </div>
        <div className="flex-1 min-w-48">
          <label className="block text-xs font-medium text-gray-700 mb-1">Tên nhóm KPI *</label>
  ...
  return (
    <div className="card border-blue-200 bg-blue-50 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Mã KPI (tự động)</label>
          <input className="input bg-gray-100 text-gray-500 text-xs font-mono" value={form.kpi_id} readOnly />
        </div>
        <div className="lg:col-span-2">
          <label className="block text-xs font-medium text-gray-700 mb-1">Tên KPI *</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Chiều KPI</label>
        ...
        <div className="lg:col-span-4">
          <label className="block text-xs font-medium text-gray-700 mb-1">Cách tính</label>
...
function KpiEditForm({ cap, nhomList, initial, onSave, onCancel }) {
  ...
  return (
    <div className="flex items-end gap-3 flex-wrap py-1">
      <div className="text-xs text-gray-500 self-center shrink-0 hidden sm:block">
        <span className="font-medium text-gray-700">{form.ten_kpi || form.kpi_id}</span>
        <span className="block text-gray-400">Để sửa Tên/Đơn vị tính/Chiều → Thư viện KPI</span>
      </div>
      <div className="w-52">
        ...
        {phase === 1 ? (
              ...
              {catalog.length === 0 ? (
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {catalog.map(item => (
                      <tr key={item.nhom_id}
                        className={`border-t border-gray-100 hover:bg-gray-50 cursor-pointer ${selected.has(item.nhom_id) ? 'bg-green-50' : ''}`}
          ...
          <>
            <div className="px-4 py-3 border-b">
              <p className="text-sm text-gray-600">Gán STT (La Mã) cho {assignments.length} nhóm đã chọn:</p>
            </div>
            <div className="overflow-y-auto flex-1">
              {assignments.length === 0
                ? <p className="text-center text-gray-400 py-8 text-sm">Chưa có nhóm nào</p>
                : (
                  <table className="w-full text-xs">
                    <thead className="bg-blue-50 border-b border-blue-100 sticky top-0">
                      <tr>
                    ...
                    <tbody>
                      {assignments.map(a => {
                        ...
                        return (
                          <tr key={a.nhom.nhom_id} className="border-t border-gray-100">
        ...
        {phase === 1 ? (
              ...
              {libItems.length === 0 ? (
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {libItems.map(item => (
                      <tr key={item.kpi_id}
                        className={`border-t border-gray-100 hover:bg-gray-50 cursor-pointer ${selected.has(item.kpi_id) ? 'bg-blue-50' : ''}`}
                        ...
                        <td className="td text-gray-500">{item.don_vi}</td>
                        <td className="td text-center text-gray-500">{item.upper_gt_lower ? '↑' : '↓'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <>
            <div className="px-4 py-3 border-b flex items-center gap-3 flex-wrap">
              ...
              <span className="text-sm text-gray-600">{assignments.length} KPI đã chọn</span>
              {nhomOptions.length > 0 && assignments.length > 1 && (
                <div className="flex items-center gap-2 ml-auto">
                  <span className="text-xs text-gray-500 whitespace-nowrap">Gán nhóm tất cả:</span>
            ...
            <div className="overflow-y-auto overflow-x-auto flex-1">
              {assignments.length === 0
                ? <p className="text-center text-gray-400 py-8 text-sm">Chưa có KPI nào</p>
                : (
                  <table className="w-full text-xs">
                    <thead className="bg-blue-50 border-b border-blue-100 sticky top-0">
                      <tr>
                    ...
                    <tbody>
                      {assignments.map(a => (
                        <tr key={a.kpi.kpi_id} className="border-t border-gray-100">
                          <td className="td font-medium">
                            {a.kpi.ten_kpi}
                            <span className="block text-[10px] text-gray-400">{a.kpi.don_vi}{a.kpi.upper_gt_lower ? ' ↑' : ' ↓'}</span>
                          </td>
                          <td className="td">
...
function KpiGroupTable({ list, nhomMap, cap, nhomList, editingKpi, setEditingKpi, onSave, onDelete, colSpan = 5, sort }) {
  ...
  if (sortedList.length === 0) {
    return (
      <tr>
        <td colSpan={colSpan} className="td text-center text-gray-400 py-8">
  ...
  return groupIds.map(nhom_id => {
    ...
    return [
      ...
      ...groupMap[nhom_id].map(k => {
        ...
        return [
          <tr key={k.kpi_id}
            className={`border-t border-gray-100 hover:bg-gray-50 ${isEditingThis ? 'bg-blue-50' : ''}`}>
            <td className="td text-center text-gray-400 font-mono w-8">{k.stt}</td>
            <td className="td text-gray-900 font-medium">
              {k.ten_kpi}
              <span className="block text-gray-400 font-mono font-normal text-[10px]">{k.kpi_id}</span>
            </td>
            <td className="td text-gray-500 w-14">{k.don_vi}</td>
            <td className="td text-gray-600 w-28">{k.upper_gt_lower ? '↑ Cao hơn tốt hơn' : '↓ Thấp hơn tốt hơn'}</td>
            {onDelete && (
              <td className="td text-center w-20">
                <div className="flex gap-1 justify-center">
                  <button onClick={() => setEditingKpi(isEditingThis ? null : { ...k })}
                    className={`p-1 rounded hover:bg-blue-50 ${isEditingThis ? 'text-blue-600' : 'text-gray-400 hover:text-blue-600'}`}
...
function ThuVienKPI({ kpiList, nhomList, kpiLibrary, nhomLibrary, persistKpi, persistNhom, persistKpiLibrary, persistNhomLibrary }) {
  ...
  const nhomCatalog = useMemo(() =>
    getAllNhomCatalog()
      ...
      .filter(n => !search || n.ten_nhom.toLowerCase().includes(search.toLowerCase()) || n.nhom_id.toLowerCase().includes(search.toLowerCase())),
    [nhomLibrary, cap, search, nhomList]
  );
  ...
  const handleImportNhom = e => {
    ...
    parseImportNhomKpi(file, entries => {
      ...
      let doUpdates = toUpdate.length > 0;
      if (doUpdates) {
        const lines = toUpdate.slice(0, 10).map(u => `• [${u.nhom_id}] "${u.ten_nhom_cu}" → "${u.ten_nhom_moi}"`).join('\n');
        const more  = toUpdate.length > 10 ? `\n... và ${toUpdate.length - 10} nhóm khác` : '';
        doUpdates = window.confirm(
          `⚠️ Phát hiện ${toUpdate.length} nhóm đã tồn tại sẽ được CẬP NHẬT tên:\n\n${lines}${more}\n\n` +
          `Thay đổi tên nhóm ảnh hưởng đến hiển thị báo cáo KPI.\n\n` +
          `OK = cập nhật + thêm mới | Hủy = chỉ thêm mới, bỏ qua cập nhật.`
        );
      }
  ...
  const handleImportKpi = e => {
    ...
    parseImportKpiLib(file, entries => {
      ...
      let doUpdates = toUpdate.length > 0;
      if (doUpdates) {
        const lines = toUpdate.slice(0, 8).map(({ existing: ex, entry: en }) => {
          const diffs = [];
        ...
        const more = toUpdate.length > 8 ? `\n... và ${toUpdate.length - 8} KPI khác` : '';
        doUpdates = window.confirm(
          `⚠️ Phát hiện ${toUpdate.length} KPI đã tồn tại sẽ được CẬP NHẬT:\n\n${lines}${more}\n\n` +
          `Thay đổi thông tin KPI ảnh hưởng đến hiển thị báo cáo và template.\n\n` +
          `OK = cập nhật + thêm mới | Hủy = chỉ thêm mới, bỏ qua cập nhật.`
        );
      }
      ...
      if (doUpdates) {
        ...
        // Nếu KPI đang trong template, cập nhật resolved fields trong kpiList
        if (updateCount > 0) {
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h3 className="font-semibold text-sm text-gray-700">
            ...
            <span className="text-gray-400 font-normal ml-1">({nhomCatalog.length} nhóm)</span>
          </h3>
        ...
        <div className="card p-0 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {nhomSorted.length === 0 ? (
                  <tr><td colSpan={5} className="td text-center text-gray-400 py-8">Chưa có nhóm nào</td></tr>
                ) : nhomSorted.map(item => {
                  const isEditing = editingNhom?.nhom_id === item.nhom_id;
                  const canDelete = !item.inTemplate && item.usedInMonths.length === 0;
                  return [
                    <tr key={item.nhom_id} className={`border-t border-gray-100 hover:bg-gray-50 ${isEditing ? 'bg-green-50' : ''}`}>
                      ...
                      <td className="td hidden md:table-cell">
                        {item.usedInMonths.length > 0
                          ? <span className="whitespace-pre-line text-gray-600">{formatUsedMonths(item.usedInMonths)}</span>
                          : <span className="text-gray-400 italic">Chưa dùng</span>}
                      </td>
                      ...
                      <td className="td text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => setEditingNhom(isEditing ? null : item)}
                            className={`p-1.5 rounded-lg transition-colors ${isEditing ? 'text-blue-600 bg-blue-100' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'}`}
                          ...
                          {canDelete ? (
                            <button onClick={() => handleNhomDelete(item.nhom_id, item.ten_nhom)}
                              className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Xóa vĩnh viễn">🗑️</button>
                          ) : (
                            <span className="p-1.5 text-gray-300 cursor-not-allowed" title={item.inTemplate ? 'Đang trong template' : 'Đã dùng trong tháng'}>🔒</span>
                          )}
                        </div>
                      </td>
                    </tr>,
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h3 className="font-semibold text-sm text-gray-700">
            ...
            <span className="text-gray-400 font-normal ml-1">({kpiCatalog.length} KPI)</span>
          </h3>
          <div className="flex gap-2 flex-wrap items-center">
        ...
        <div className="card p-0 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {kpiSorted.length === 0 ? (
                  <tr><td colSpan={8} className="td text-center text-gray-400 py-8">Chưa có KPI nào trong thư viện</td></tr>
                ) : kpiSorted.map(item => {
                  ...
                  return [
                    <tr key={item.kpi_id} className={`border-t border-gray-100 hover:bg-gray-50 ${isEditing ? 'bg-blue-50' : isExpanded ? 'bg-amber-50' : ''}`}>
                      <td className="td font-medium text-gray-900">
                        {item.ten_kpi}
                      ...
                      <td className="td text-gray-500 hidden sm:table-cell">{item.don_vi}</td>
                      <td className="td text-gray-600 hidden lg:table-cell">{item.upper_gt_lower ? '↑ Cao hơn tốt hơn' : '↓ Thấp hơn tốt hơn'}</td>
                      <td className="td hidden md:table-cell">
                        {monthStr
                          ? <span className="whitespace-pre-line text-gray-600 leading-relaxed">{monthStr}</span>
                          : <span className="text-gray-400 italic">Chưa dùng</span>}
                      </td>
                      ...
                      <td className="td text-center hidden sm:table-cell">
                        {hasCachTinh ? (
                          <button
                            className={`text-xs px-2 py-0.5 rounded border transition-colors ${isExpanded ? 'bg-amber-100 text-amber-700 border-amber-300' : 'bg-gray-50 text-blue-600 border-blue-200 hover:bg-blue-50'}`}
                    ...
                    isExpanded && (
                      <tr key={`ct-${item.kpi_id}`} className="bg-amber-50 border-t border-amber-200">
                        <td colSpan={8} className="px-4 py-3">
                          <p className="text-xs font-medium text-amber-800 mb-1">Cách tính: <span className="font-normal text-gray-700">{item.ten_kpi}</span></p>
                          <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{item.cach_tinh}</p>
                        </td>
                      </tr>
                    ),
                    isEditing && (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-3 space-y-1">
          ...
          {!items.length && <p className="text-gray-400 text-sm text-center py-4">Không có mục nào</p>}
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        <span className="text-xs text-gray-400 whitespace-nowrap">{filtered.length} KPI</span>
      </div>

      {/* Nhóm hiện có trong template */}
      {nhomListCap.length > 0 && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 space-y-2">
          <p className="text-xs font-semibold text-gray-600">Nhóm trong template:</p>
          <div className="flex flex-wrap gap-2">
            {nhomListCap.map(n => (
              <div key={n.nhom_id} className={`flex items-center gap-1 bg-white border rounded-lg px-2 py-1 text-xs ${editingNhom?.nhom_id === n.nhom_id ? 'border-blue-400 ring-1 ring-blue-200' : 'border-gray-200'}`}>
                ...
                <button onClick={() => setEditingNhom(editingNhom?.nhom_id === n.nhom_id ? null : { nhom_id: n.nhom_id, ten_nhom: n.ten_nhom, thu_tu: n.thu_tu, kpi_cap: n.kpi_cap })}
                  className="text-gray-300 hover:text-blue-500 ml-1" title="Chỉnh sửa">✏️</button>
                <button onClick={() => removeNhom(n.nhom_id)} className="text-gray-300 hover:text-red-500" title="Xóa">✕</button>
              </div>
            ))}
          </div>
          {editingNhom && nhomListCap.some(n => n.nhom_id === editingNhom.nhom_id) && (
            <div className="flex gap-2 items-end flex-wrap bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 mt-1">
              <div>
                <label className="block text-xs text-gray-500 mb-0.5">STT La Mã</label>
              ...
              <p className="text-xs text-gray-500 self-center">
                Tên nhóm: <span className="font-medium text-gray-700">{editingNhom.ten_nhom}</span>
                <span className="ml-1 text-gray-400">(sửa tên trong Thư viện KPI)</span>
              </p>
...
function TheoThang({ kpiList, nhomList }) {
  ...
  const deleteMonth = async () => {
    ...
    syncToGas('input_phong_' + thang, null);
    syncToGas('output_diem_' + thang, null);
    syncToGas('output_meta_' + thang, null);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {snapList.length > 0 ? (
          ...
          <span className="text-sm text-gray-400 italic">Chưa có tháng nào</span>
        )}
      ...
      {snapshot && (
        <>
          ...
          {editMode && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 space-y-2">
              <p className="text-xs font-semibold text-gray-600">Nhóm trong tháng {capTab === 'ca_nhan' ? 'cá nhân' : 'phòng'}:</p>
              <div className="flex flex-wrap gap-2">
                {editedNhomList.filter(n => n.kpi_cap === capTab)
                  ...
                  .map(n => (
                    <div key={n.nhom_id} className={`flex items-center gap-1 bg-white border rounded-lg px-2 py-1 text-xs ${editingNhom?.nhom_id === n.nhom_id ? 'border-blue-400 ring-1 ring-blue-200' : 'border-gray-200'}`}>
                      ...
                      <button onClick={() => setEditingNhom(editingNhom?.nhom_id === n.nhom_id ? null : { nhom_id: n.nhom_id, ten_nhom: n.ten_nhom, thu_tu: n.thu_tu, kpi_cap: n.kpi_cap })}
                        className="text-gray-300 hover:text-blue-500 ml-1" title="Chỉnh sửa">✏️</button>
                      <button onClick={() => handleRemoveNhom(n.nhom_id)} className="text-gray-300 hover:text-red-500" title="Xóa">✕</button>
                    </div>
                  ))}
                {editedNhomList.filter(n => n.kpi_cap === capTab).length === 0 && (
                  <span className="text-xs text-gray-400 italic">Chưa có nhóm — thêm từ thư viện</span>
                )}
              </div>
...
export default function KPIManagement() {
  ...
  const { user } = useAuth();
  if (!canEditDept(user)) return <div className="p-3 md:p-6"><h2 className="text-xl font-bold text-gray-900">Quản lý KPI</h2><div className="mt-6"><AccessDenied /></div></div>;
  ...
  return (
    <div className="p-3 md:p-6 space-y-5">
      ...
      <div>
        <h2 className="text-lg md:text-xl font-bold text-gray-900">{TAB_TITLES[urlTab] ?? 'Quản lý KPI'}</h2>
        <p className="text-gray-500 text-xs mt-0.5">Luồng xử lý dữ liệu: Tạo thư viện → Tạo template → Tạo KPI tháng</p>
      </div>
