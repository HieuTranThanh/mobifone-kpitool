 ...
 * - branch_viewer: chỉ dashboard + baocao
...
// Switcher phòng cho admin/branch_viewer — phải chọn đúng 1 phòng, không có "Tất cả"
...
export default function Layout() {
  ...
  // Admin/branch_viewer: phải chọn phòng trước khi xem data; '' = chưa chọn
  ...
  const toggleMenu = (base, firstChildTo) => {
    if (expandedBase === base) {
      ...
      if (!checkNavGuard(firstChildTo)) return;
  ...
  useEffect(() => {
    // Global viewers (admin/branch_viewer): chờ chọn phòng trước khi load
  ...
  const sidebarContent = (
    <>
          ...
          if (!item.children) {
            return (
              <NavLink
                ...
                onClick={(e) => { if (!checkNavGuard(item.to)) { e.preventDefault(); return; } closeSidebar(); }}
                className={({ isActive }) =>
                ...
                <div className="ml-3 mt-0.5 space-y-0.5 border-l border-slate-700/60 pl-2">
                  {item.children.map((child, childIdx) => (
                    <NavLink
                      key={child.to}
                      to={child.to}
                      onClick={(e) => { if (!checkNavGuard(child.to)) { e.preventDefault(); return; } closeSidebar(); }}
                      className={({ isActive }) =>
      ...
      <div className="px-2 py-3 border-t border-slate-700/50 space-y-0.5 shrink-0">
        ...
        <p className="text-xs text-slate-600 px-3">© {new Date().getFullYear()} Hieu.TranThanh</p>
      </div>
    </>
  );
