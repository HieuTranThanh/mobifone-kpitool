 ...
 * - trong_so_weights_YYYY-MM → syncToSupabase (Supabase config_store) — chỉ khi mode='auto'
 ...
 *
 * LƯU Ý:
 * - Chuyển auto→manual: phải deleteFromSupabase('trong_so_weights_*') để xóa key trong config_store.
...
import {
  ...
  syncToSupabase, deleteFromSupabase,
  getInputCNByThang, upsertInputCN,
} from '../services/store';
import { isConnected, syncWeightConfig, getInputCN as sbGetInputCN } from '../services/supabaseService';
...
function normalizePriorities(config) {
  // Migrate key cũ ty_le.phong.cty → chinhanh (giữ tương thích cấu hình tháng cũ đã lưu)
  let cfg = config;
  const ph = config.ty_le?.phong;
  if (ph && ph.chinhanh === undefined && ph.cty !== undefined) {
    cfg = { ...config, ty_le: { ...config.ty_le, phong: { chinhanh: ph.cty, phong: ph.phong } } };
  }
  if (!cfg.cv_priorities) return cfg;
  const newCvPrio = {};
  Object.entries(cfg.cv_priorities).forEach(([nhomCv, prioMap]) => {
  ...
  return { ...cfg, cv_priorities: newCvPrio };
}
...
function TyLeNumInput({ value, onChange: onCh, readOnly }) {
  return (
    <input type="number" min="0" max="100" step="5"
      className={`input w-16 text-center ${readOnly ? 'bg-slate-50 cursor-not-allowed' : ''}`}
      value={value}
      disabled={readOnly}
...
function TyLeCap({ config, kpiList, onChange, readOnly }) {
  ...
  const update = (group, side, raw) => {
    ...
    const newTL = { ...ty_le };
    if (group === 'phong')
      newTL.phong = side === 'chinhanh' ? { chinhanh: val, phong: other } : { chinhanh: other, phong: val };
    else
  ...
  return (
    <div className="card p-4">
      <h3 className="font-semibold text-slate-800 mb-3">Bước 1 — Cấu hình tỷ lệ trọng số giữa các cấp</h3>
      <div className="space-y-3 text-sm">
        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
          <span className="font-medium text-slate-700 sm:w-36">Điểm KPI phòng =</span>
          <div className="flex items-center gap-2 flex-wrap">
            <TyLeNumInput value={ty_le.phong.chinhanh} onChange={v => update('phong', 'chinhanh', v)} readOnly={readOnly} />
            ...
            <span className="text-slate-400 text-xs">(tổng {ty_le.phong.chinhanh + ty_le.phong.phong}%)</span>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
          <span className="font-medium text-slate-700 sm:w-36">Điểm KPI cá nhân =</span>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-slate-400 text-xs">(tổng {ty_le.ca_nhan.phong + ty_le.ca_nhan.ca_nhan}%)</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="card p-4 space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h3 className="font-semibold text-slate-800">Bước 3 — Tạo nhóm KPI và trọng số tương ứng</h3>
      ...
      <p className="text-xs text-slate-500">Tạo nhóm để phân loại KPI. Tổng % các nhóm phải bằng {ca_nhan_pct}đ (phần cá nhân).</p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {config.nhom_kpi.map(nhom => (
              <tr key={nhom.id} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="td">
                  {!readOnly && renamingId === nhom.id ? (
                    ...
                    <span className="font-medium text-slate-800">{nhom.ten}</span>
                  )}
                </td>
                <td className="td text-center">
                  <div className="flex items-center justify-center gap-1">
                    <input type="number" min="0" max={ca_nhan_pct} step="1"
                      className={`input w-16 text-center text-sm py-0.5 ${readOnly ? 'bg-slate-50 cursor-not-allowed' : ''}`}
                      value={nhom.pct}
                      disabled={readOnly}
      ...
      {config.nhom_kpi.length === 0 && (
        <p className="text-slate-400 text-sm italic">Chưa có nhóm KPI nào.</p>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col">
        <div className="px-5 py-4 border-b flex items-center justify-between">
          <h3 className="font-bold text-slate-800">+ Thêm KPI vào nhóm "{nhomKpiTen}"</h3>
        ...
        {phase === 1 ? (
          <>
            ...
            <div className="overflow-y-auto flex-1">
              {filtered.length === 0 ? (
                <p className="text-center text-slate-400 py-10 text-sm">Không có KPI nào chưa được gán.</p>
              ) : (
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {filtered.map(kk => (
                      <tr key={kk.kpi_id}
                        className={`border-t border-slate-100 hover:bg-slate-50 cursor-pointer ${selected.has(kk.kpi_id) ? 'bg-blue-50' : ''}`}
                        ...
                        <td className="td font-medium text-slate-700">{kk.ten_kpi}</td>
                        <td className="td text-center text-slate-400">{kk.stt}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <>
            <div className="px-4 py-3 border-b flex items-center gap-3">
              ...
              <span className="text-sm text-slate-600">{assignments.length} KPI — đặt mức ưu tiên</span>
            </div>
            <div className="overflow-y-auto flex-1">
              <table className="w-full text-xs">
                <thead className="bg-blue-50 border-b border-blue-100 sticky top-0">
                ...
                <tbody>
                  {assignments.map(a => (
                    <tr key={a.kpiId} className="border-t border-slate-100">
                      <td className="td text-slate-700 font-medium">{a.tenKpi}</td>
                      <td className="td text-center">
                        <select
                      ...
                      <td className="td text-center">
                        <button className="text-slate-300 hover:text-red-500" onClick={() => removeItem(a.kpiId)}>✕</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function exportNhomWeightsToExcel(thang, nhomCvList, config, kpiList) {
  ...
  nhomCvList.forEach(nhomCv => {
    ...
    ws['!rows'] = [{ hpt: 25 }, ...rows.map(() => ({ hpt: 36 }))];
    ...
    rows.forEach((row, ri) => {
      ...
      row.forEach((val, ci) => {
        ...
        ws[addr] = {
          ...
          s: { ...ds, alignment: { ...ds.alignment, horizontal: ci === 1 ? 'left' : 'center', ...(ci === 1 && { wrapText: true }) } },
        };
      });
    });
...
function CvConfigPanel({ thang, config, nhomCvList, kpiList, onChange, readOnly }) {
  ...
  if (!config.nhom_kpi.length) {
    return (
      <div className="card p-4">
        <h3 className="font-semibold text-slate-800 mb-2">Bước 4 — Cấu hình KPI theo nhóm công việc</h3>
        <p className="text-slate-400 text-sm italic">Tạo nhóm KPI ở Bước 3 trước.</p>
      </div>
    );
  }
    ...
    <div className="card p-4 space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          <h3 className="font-semibold text-slate-800">Bước 4 — Cấu hình KPI theo nhóm công việc</h3>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg px-3 py-2.5 space-y-2">
        <div className="flex items-center gap-4 flex-wrap">
          <span className="text-sm text-slate-700 font-medium whitespace-nowrap">Phân bổ tự động:</span>
          <div className="flex items-center gap-2">
            <label className="text-xs text-slate-500 whitespace-nowrap">Điểm KPI cao nhất:</label>
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs text-slate-500 whitespace-nowrap">Điểm KPI thấp nhất:</label>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-600 space-y-1">
        <p className="font-medium text-slate-700">Cách sử dụng cột "Mức ưu tiên":</p>
        <div className="grid grid-cols-1 gap-0.5">
          ...
          <span><span className="inline-block w-5 text-center text-slate-400">—</span> <strong>Không tính:</strong> trọng số = 0đ, KPI không được chấm điểm tháng này.</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {summary.map(({ cv, tot }) => {
          ...
          return (
            <span key={cv}
              className={`text-xs px-2 py-0.5 rounded border cursor-pointer ${cv === selNhomCv ? 'bg-blue-100 text-blue-700 border-blue-300' : isOk ? 'bg-green-50 text-green-600 border-green-200' : isOvr ? 'bg-red-50 text-red-600 border-red-200' : 'bg-slate-50 text-slate-500 border-slate-200'}`}
      ...
      <div className="space-y-4">
        {config.nhom_kpi.map(nhom => {
          ...
          return (
            <div key={nhom.id} className="border border-slate-200 rounded-lg p-3 bg-slate-50 space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-medium text-slate-800 text-sm">{nhom.ten}</span>
                {nhomBudget > 0 && (
                  <span className="text-xs text-slate-400 bg-white border border-slate-200 px-2 py-0.5 rounded">{nhomBudget}đ tham khảo</span>
                )}
                {nhomKpis.length > 0 && (
                  <span className="text-xs px-1.5 py-0.5 rounded text-slate-500">phân bổ thực tế: {fmt2(nhomTotal)}đ</span>
                )}
              </div>

              {nhomKpis.length > 0 && (
              <div className="overflow-x-auto">
                <table className="w-full text-xs border border-slate-100 rounded bg-white">
                  <thead className="bg-blue-50 border-b border-blue-100">
                    <tr>
                  ...
                  <tbody>
                    {nhomKpis.map(kk => {
                      ...
                      return (
                        <tr key={kk.kpi_id} className="border-t border-slate-100 hover:bg-slate-50">
                          <td className="td text-center">
                            <select
                              className={`input text-xs py-0.5 w-28 text-center ${isZero ? 'text-slate-300' : isFixed ? 'text-orange-700 font-semibold' : 'text-blue-700 font-semibold'} ${readOnly ? 'bg-slate-50 cursor-not-allowed' : ''}`}
                              value={currentPrio ?? ''}
                              disabled={readOnly}
                              onChange={e => {
                                if (readOnly) return;
                          ...
                          <td className="td text-slate-700">{kk.ten_kpi}</td>
                          <td className="td text-center">
                            {!readOnly && isEdit ? (
                              ...
                              <button
                                className={`text-xs px-2 py-0.5 rounded font-mono ${isZero ? 'bg-slate-100 text-slate-300 cursor-default' : isFixed ? 'bg-orange-100 text-orange-700' : isCustom ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}`}
                          ...
                          <td className="td text-center">
                            {!readOnly && (
                              <div className="flex items-center justify-center gap-1">
                                {isCustom && isAuto && (
                                  <button className="text-slate-300 hover:text-slate-500" onClick={() => resetKpi(kk.kpi_id)} title="Đặt lại về tự động">↺</button>
                                )}
                                <button className="text-slate-300 hover:text-red-500" onClick={() => removeKpi(nhom.id, kk.kpi_id)} title="Bỏ KPI khỏi nhóm">×</button>
                              </div>
                            )}
                          </td>
                        </tr>
              ...
              {nhomKpis.length === 0 && (
                <p className="text-xs text-slate-400 italic">Chưa có KPI nào trong nhóm này.</p>
              )}
              ...
              {!readOnly && unassigned.length > 0 ? (
                ...
                <p className="text-xs text-slate-300 italic">Tất cả KPI đã được gán vào nhóm khác.</p>
              ) : null}
            </div>
          );
        })}
...
function ModeSelector({ config, onChange, readOnly, onQuickSwitch }) {
  const mode = config.mode || 'manual';
  const modeLabels = { manual: '✋ Nhập tay', auto: '⚙️ Tự động' };

  return (
    <div className="card p-4">
      <h3 className="font-semibold text-slate-800 mb-3">Bước 2 — Chế độ nhập trọng số</h3>
      {readOnly ? (
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {onQuickSwitch && (
            <button className="btn-secondary text-xs" onClick={onQuickSwitch}>
              🔄 Đổi chế độ
            </button>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex gap-3 flex-wrap">
            {[['manual', '✋ Nhập tay', 'nhập hoặc import từng NV'], ['auto', '⚙️ Tự động', 'tính từ cấu hình Bước 3–4']].map(([m, label, desc]) => (
              <label key={m} className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer ${mode === m ? 'border-blue-400 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                <input type="radio" name="mode" value={m} checked={mode === m} onChange={() => onChange({ ...config, mode: m })} className="accent-blue-600" />
          ...
          <p className="text-xs text-slate-500">
            {mode === 'auto'
              ...
              : '⚠️ Chế độ nhập tay. Trọng số tự động (nếu có) sẽ bị xóa khi bấm Lưu & Sync — calcMonth đọc từ cột _trong_so trên sheet.'}
          </p>
        </div>
      )}
    </div>
    ...
    <div className="relative" tabIndex={-1}
      ...
      <button type="button"
        ...
        <span className="flex-1 truncate">
          {selected ? selected.ho_ten : <span className="text-slate-400">{placeholder}</span>}
        </span>
        <span className="text-slate-400 shrink-0 ml-1">▾</span>
      </button>
      {open && (
        <div className="absolute top-full left-0 z-30 bg-white border border-slate-200 rounded-lg shadow-xl mt-1 w-64">
          <input autoFocus
          ...
          <div className="max-h-48 overflow-y-auto">
            {filtered.map(nv => (
              <div key={nv.nv_id} tabIndex={0}
                className={`px-3 py-1.5 text-xs cursor-pointer hover:bg-blue-50 ${nv.nv_id === value ? 'bg-blue-100 text-blue-700 font-medium' : 'text-slate-700'}`}
                ...
                {(nv.nhom_cv || nv.khu_vuc) && (
                  <div className="text-[10px] text-slate-400">{[nv.nhom_cv, nv.khu_vuc].filter(Boolean).join(' · ')}</div>
                )}
              </div>
            ))}
            {filtered.length === 0 && <p className="px-3 py-2 text-xs text-slate-400 italic">Không tìm thấy.</p>}
          </div>
        </div>
      )}
    </div>
...
function SearchableNvMultiSelect({ nvList, values, onChange, placeholder = 'Chọn nhân viên đích...' }) {
  ...
  return (
    <div className="relative" tabIndex={-1}
      ...
      {open && (
        <div className="absolute top-full left-0 z-30 bg-white border border-slate-200 rounded-lg shadow-xl mt-1 w-72">
          <input autoFocus
            className="input text-xs m-1.5 block w-[calc(100%-12px)]"
            placeholder="Tìm tên, nhóm CV, khu vực..."
            value={search}
            onChange={e => setSearch(e.target.value)} />
          <div className="flex items-center gap-3 px-3 py-1 border-b border-slate-100 text-xs">
            <button tabIndex={0} className="text-blue-500 hover:text-blue-700" onClick={selectAllFiltered}>Chọn tất cả</button>
            <button tabIndex={0} className="text-slate-400 hover:text-slate-600" onClick={clearFiltered}>Bỏ chọn</button>
            {values.length > 0 && <span className="ml-auto text-slate-400">{values.length} đã chọn</span>}
          </div>
          <div className="max-h-48 overflow-y-auto">
            {filtered.map(nv => (
              <label key={nv.nv_id} tabIndex={0}
                ...
                <div>
                  <div className="text-slate-700">{nv.ho_ten}</div>
                  {(nv.nhom_cv || nv.khu_vuc) && (
                    <div className="text-[10px] text-slate-400">{[nv.nhom_cv, nv.khu_vuc].filter(Boolean).join(' · ')}</div>
                  )}
                </div>
...
function ManualWeightGrid({ thang, config, kpiList, nvList }) {
  ...
  useEffect(() => { setGrid(initGrid()); }, [thang, nvList.length, kpiList.length]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="card p-4 space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h3 className="font-semibold text-slate-800">Trọng số cá nhân — chế độ nhập tay</h3>
      ...
      {nvList.length > 0 && (
        <div className="flex gap-2 flex-wrap items-center text-xs">
          ...
          <span className="text-slate-400">Hiển thị {displayNvList.length}/{nvList.length} NV</span>
        </div>
      )}
      ...
      {kpiList.length === 0 ? (
        <p className="text-slate-400 text-sm italic">Tháng này chưa có KPI cá nhân.</p>
      ) : displayNvList.length === 0 ? (
        <p className="text-slate-400 text-sm italic">Không có nhân viên phù hợp bộ lọc.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-blue-50 border-b border-blue-100">
              <tr>
                ...
                {displayNvList.map(nv => (
                  <th key={nv.nv_id} className="th text-center min-w-[84px]">
                    <div>{nv.ho_ten}</div>
                    {nv.nhom_cv && <div className="text-slate-400 font-normal text-[10px]">{nv.nhom_cv}</div>}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {kpiList.map(k => (
                <tr key={k.kpi_id} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className="td text-center text-slate-400">{k.stt}</td>
                  <td className="td text-slate-700">{k.ten_kpi}</td>
                  {displayNvList.map(nv => (
      ...
      <div className="card p-4 space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {nv && <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded">{nv.nhom_cv}</span>}
          </div>
        </div>
        ...
        {nv ? (
          <>
            ...
            {displayKpis.length === 0 ? (
              <p className="text-slate-400 text-sm italic">Nhân viên này chưa có KPI được phân bổ trọng số (cần cấu hình Bước 3–4 trước).</p>
            ) : (
              <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-blue-50 border-b border-blue-100">
                ...
                <tbody>
                  {displayKpis.map(k => {
                    ...
                    return (
                      <tr key={k.kpi_id} className={`border-t border-slate-100 ${isOverride ? 'bg-orange-50' : 'hover:bg-slate-50'}`}>
                        <td className="td text-center text-slate-400">{k.stt}</td>
                        <td className="td text-slate-700">{k.ten_kpi}</td>
                        <td className="td text-center font-mono text-slate-400">{fmt2(tmplPct)}đ</td>
                        <td className="td text-center">
                          {!readOnly && isEdit ? (
                            <input autoFocus type="number" step="0.1" min="0" max={ca_nhan_pct}
                            ...
                            <button
                              className={`text-xs px-2 py-0.5 rounded font-mono ${isOverride ? 'bg-orange-100 text-orange-700 font-semibold' : 'bg-slate-100 text-slate-400'} ${!readOnly ? 'hover:bg-slate-200 cursor-pointer' : 'cursor-default'}`}
                        ...
                        <td className="td text-center">
                          {!readOnly && isOverride && (
                            <button className="text-slate-300 hover:text-slate-500 text-xs" onClick={() => clearOvrd(k.kpi_id)} title="Xóa ghi đè">↺</button>
                          )}
                        </td>
                      </tr>
                    );
          ...
          <p className="text-slate-400 text-sm italic">Chọn nhân viên để cấu hình.</p>
        )}
      </div>
    </div>
  );
...
export default function WeightManagement() {
  ...
  // Snapshot config khi bắt đầu edit — để revert localStorage nếu user discard
  const originalConfigRef = useRef(null);
  // Ref tránh stale closure trong unmount cleanup
  const editCleanupRef    = useRef({ editMode: false, isDirty: false, thang: '' });
  ...
  // Sync state vào ref để unmount cleanup đọc được giá trị mới nhất
  useEffect(() => {
    editCleanupRef.current = { editMode, isDirty, thang };
  }, [editMode, isDirty, thang]);

  // Khi component unmount trong trạng thái dirty (user navigate đi mà không save):
  // revert localStorage về config gốc để discard thực sự có hiệu lực
  useEffect(() => {
    return () => {
      const { editMode, isDirty, thang } = editCleanupRef.current;
      if (editMode && isDirty && originalConfigRef.current) {
        localStorage.setItem(`trong_so_thang_${thang}`, JSON.stringify(originalConfigRef.current));
      }
    };
  }, []);
  ...
  const handleDiscard = () => {
    if (!window.confirm('Bỏ tất cả thay đổi chưa lưu?')) return;
  ...
  const loadThang = (t) => {
    if (editMode && isDirty) {
      ...
      // Revert localStorage về config gốc khi user confirm discard-by-switching-month
      if (originalConfigRef.current) {
        localStorage.setItem(`trong_so_thang_${thang}`, JSON.stringify(originalConfigRef.current));
      }
    }
  ...
  }, [thang, thangList]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleSave = async () => {
    ...
    if (config.mode === 'auto' && isConnected() && kpiList.length > 0) {
      try {
        ...
        syncToSupabase(`trong_so_weights_${thang}`, weights);
  ...
  const handleQuickModeSwitch = () => {
    if (!config || !thang) return;
    const newMode = config.mode === 'auto' ? 'manual' : 'auto';
    const label   = newMode === 'auto' ? 'tự động' : 'bằng tay';
    const note    = newMode === 'auto'
      ? '\n\nLưu ý: Trọng số chưa được tính. Bạn cần vào Chỉnh sửa → cấu hình Bước 3–4 → Lưu & Sync để hệ thống tính trọng số.'
      : '\n\nTrọng số tự động hiện tại sẽ bị xóa. Bạn cần nhập trọng số thủ công ở tab Trọng số cá nhân.';
    if (!confirm(`Chuyển sang chế độ nhập trọng số ${label}?${note}`)) return;
    const newConfig = { ...config, mode: newMode };
    setConfig(newConfig);
    saveTrongSoConfig(thang, newConfig);
    if (newMode === 'manual') {
      localStorage.removeItem(`trong_so_weights_${thang}`);
      deleteFromSupabase(`trong_so_weights_${thang}`);
    }
    setSaveStatus(`✅ Đã chuyển sang chế độ ${label}`);
  ...
  const copyFromMonth = () => {
    ...
    const base = normalizePriorities({
      ...
      // kpi_pct giữ nguyên từ srcCfg để recomputeAllKpiPct dùng pct cũ cho KPI 'fixed'
      nv_override:   {},
    });
    // Chỉ lưu original khi chưa trong editMode; nếu đã editMode thì giữ snapshot từ handleEnterEdit
    if (!editMode) originalConfigRef.current = config;
  ...
  if (!canEditDept(user)) return <div className="p-3 md:p-6"><h2 className="text-xl font-bold text-slate-900">Quản lý trọng số</h2><div className="mt-6"><AccessDenied /></div></div>;

  if (!thangList.length) {
    return (
      <div className="p-3 md:p-6">
        <h2 className="text-xl font-bold text-slate-900 mb-2">Quản lý Trọng số KPI</h2>
        <p className="text-slate-500">Chưa có tháng nào. Hãy tạo tháng trong module Quản lý KPI trước.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-3 md:p-6 space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg md:text-xl font-bold text-slate-900">{TAB_TITLES_TS[urlTab] ?? 'Quản lý Trọng số'}</h2>
          <p className="text-slate-500 text-xs mt-0.5">Cấu hình trọng số KPI theo nhóm công việc/cá nhân từng tháng</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {thangList.length > 1 && (
            <div className="relative">
              <button
                ...
                onClick={() => {
                  if (!showCopyPicker) {
                    ...
                    const validSrc = thangList.filter(t => t !== thang && (getTrongSoConfig(t)?.mode || 'manual') === curMode);
                    if (validSrc.length > 0 && !validSrc.includes(copySourceThang)) {
                      setCopySourceThang(validSrc[validSrc.length - 1]);
                    }
                  }
              ...
              {showCopyPicker && (() => {
                ...
                return (
                  <div className="absolute right-0 top-9 z-10 bg-white border border-slate-200 rounded-xl shadow-lg p-3 space-y-2 min-w-72">
                    <p className="text-xs font-semibold text-slate-700">Chọn tháng nguồn:</p>
                    ...
                    {sameModeSrc.length === 0 ? (
                      <p className="text-xs text-slate-400 italic">Không có tháng nào cùng chế độ để copy.</p>
                    ) : (
      ...
      {config && urlTab === 'cauhinh' && (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {!editMode ? (
              ...
              <>
                ...
                <button className="btn-secondary text-sm" onClick={handleDiscard} disabled={saving}>
                  🚫 Hủy
                </button>
              </>
            )}
          ...
          <ModeSelector config={config} onChange={onConfigChange} readOnly={!editMode} onQuickSwitch={!editMode ? handleQuickModeSwitch : undefined} />
          {config.mode === 'auto' && (
            <>
