 ...
 *
 * PHÂN QUYỀN (TODO):
 * - Staff chỉ nhập liệu cho NV của mình; quản lý nhập cho cả nhóm; admin nhập tất cả.
 * - CauHinhXepLoai: chỉ admin mới được thay đổi.
 *
 * LƯU Ý:
...
import { isConnected, syncInputCNRows, syncInputPhong, getInputCN as gasGetInputCN, calcMonth as calcGAS, upsertOutputDiem } from '../services/supabaseService';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-3">
          {kpiList.map(k => (
            <div key={k.kpi_id} className="border border-gray-200 rounded-xl p-3">
              <p className="font-semibold text-xs text-gray-800 mb-2">
                <span className="text-blue-600 font-mono mr-1">{k.stt}.</span>
                {k.ten_kpi}
                <span className="text-gray-400 ml-1">({k.don_vi})</span>
                <span className="ml-2 text-gray-400">{k.upper_gt_lower ? '↑ Cao hơn tốt hơn' : '↓ Thấp hơn tốt hơn'}</span>
              </p>
                  ...
                  return (
                    <div key={key}>
                      <label className="block text-xs mb-0.5 font-medium" style={{ color: isEmpty ? '#dc2626' : '#6b7280' }}>
                        ...
                        {roField && <span className="ml-1 text-gray-400 font-normal">(tự động)</span>}
                      </label>
                      {roField ? (
                        <div className="input text-xs py-1 bg-gray-50 text-gray-500 select-none">
...
function NhapLieuKPI() {
  ...
  const snapshot    = useMemo(() => thang ? getKpiSnapshot(thang) : null, [thang, refreshKey]);
  const isAutoMode  = (getTrongSoConfig(thang)?.mode ?? 'manual') === 'auto';
  ...
  const handleCalc = async () => {
    ...
    const incompleteNote = localUnscoredRows.length > 0
      ...
      : '';
    setCalcMsg(`⏳ Đang tính… Điểm phòng: ${diemPhong.toFixed(1)}đ${phongNote}${incompleteNote}`);
    ...
    try {
      if (isConnected()) {
        ...
        const result = await calcGAS(thang);
        // Frontend "đủ dữ liệu" (6 fields) là chuẩn — loại NV "thiếu" khỏi danh sách đã tính
        // dù calcGAS có tính được (vì server lấy trọng số từ config, bỏ qua _trong_so trong input_cn)
        ...
        // NV không có trong calcGAS và không phải incomplete
        ...
        const offlineNote = localUnscoredRows.length > 0
          ? ` ⚠️ ${localUnscoredRows.length} NV chưa đủ dữ liệu — KPI chưa được tính.`
          : '';
        setCalcMsg(`✅ Điểm phòng: ${diemPhong.toFixed(1)}đ${phongNote}. Đã tính: ${result.so_nv} NV.${offlineNote}`);
      }
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        {snapshot ? (
          <span className="text-xs text-gray-400">{kpiList.length} KPI cá nhân</span>
        ) : (
          ...
          <table className="w-full">
            ...
            <tbody>
              {sortedNv.map((n, i) => {
                ...
                return (
                  <tr key={n.nv_id} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="td text-center">
                      <span className={{ empty: 'badge bg-gray-100 text-gray-400', partial: 'badge bg-yellow-100 text-yellow-700', full: 'badge bg-green-100 text-green-700' }[inputStatus]}>
                        {{ empty: 'Chưa nhập', partial: 'Thiếu dữ liệu', full: '✓ Đủ dữ liệu' }[inputStatus]}
                      </span>
                    </td>
                    <td className="td text-center">
                      <button
                        className={`p-1.5 rounded ${locked ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'}`}
...
function NhapLieuPhong() {
  ...
  const nhomList = snapshot?.nhomList || [];
  ...
  const doSave = (formData) => {
    ...
    row.diem_kpi_chinhanh    = isNaN(kq) ? '' : Math.round(kq * ty_le_cty / 100 * 1000) / 1000;
    const kq_val   = isNaN(kq) ? 0 : kq;
    const phongSum = kpiList.reduce((s, kpi) => {
      const d = parseFloat(row[kpi.kpi_id + '_diem']);
      return s + (isNaN(d) ? 0 : d);
    }, 0);
    row.tong_diem_phong = Math.round((kq_val * ty_le_cty / 100 + phongSum) * 1000) / 1000;
  ...
  const kq_cty       = parseFloat(form.diem_kpi_chinhanh_kq) || 0;
  const diem_cty     = kq_cty * ty_le_cty / 100;
  const diemPhongSum = kpiList.reduce((s, kpi) => s + (computed[kpi.kpi_id]?.diem_quy_doi || 0), 0);
  const totalDiem    = diem_cty + diemPhongSum;
  ...
  const fmtDiem = v => v !== null ? v.toFixed(2) : '—';

  const inputCls = (locked || !editMode)
    ? 'no-spin bg-gray-50 text-gray-400 cursor-not-allowed'
    : 'no-spin';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        {snapshot ? (
          <span className="text-xs text-gray-400">{kpiList.length} KPI phòng</span>
        ) : (
          <span className="text-xs text-amber-600">⚠ Tháng {thang} chưa có snapshot KPI</span>
        )}
        {snapshot && (
			  ...
			  <table className="w-full text-sm">
				<thead className="bg-blue-50 border-b border-blue-100">
				  <tr>
					...
					<th className="th text-center w-[120px]">KQ thực hiện</th>
					<th className="th text-center w-[120px]">% Thực hiện</th>
					<th className="th text-center w-[120px]">Chỉ tiêu</th>
					<th className="th text-center w-[120px]">Ngưỡng dưới</th>
					<th className="th text-center w-[120px]">Trọng số</th>
					<th className="th text-center w-[120px]">Điểm Quy đổi</th>
					<th className="th text-center w-[120px]">Điểm tối đa (%)</th>
				  </tr>
				</thead>
				<tbody>
                {/* Row A — KPI Công ty */}
                <tr className="bg-green-50 text-green-900">
                  ...
                  <td colSpan={2} className="px-3 py-2 font-bold text-sm">KPI Công ty</td>
                  <td className="px-1.5 py-1">
                    <input
                      ...
                      className={(locked || !editMode) ? 'no-spin bg-gray-50 text-gray-400 cursor-not-allowed' : 'no-spin'}
                    />
                  </td>
                  <td className="px-3 py-2 text-center text-sm">
                    {kq_cty > 0 ? kq_cty.toFixed(1) + '%' : '—'}
                  </td>
                  <td colSpan={2} className="px-3 py-2 text-center text-xs text-green-600">—</td>
                  <td className="px-3 py-2 text-center text-sm font-semibold">{ty_le_cty}</td>
                  <td className="px-3 py-2 text-center font-bold text-green-700">{fmtDiem(diem_cty)}</td>
                  <td className="px-3 py-2 text-center text-green-600">{ty_le_cty.toFixed(2)}</td>
                </tr>
                ...
                <tr className="bg-blue-50 text-blue-900">
                  ...
                  <td className="px-3 py-2 text-center text-blue-500">{ty_le_phong.toFixed(2)}</td>
                </tr>
                ...
                {groups.map(({ nhom, kpis }) => (
                  <Fragment key={nhom.nhom_id || nhom.ten_nhom}>
                    ...
                    {kpis.map(kpi => {
                      ...
                      return (
                        <tr key={kpi.kpi_id} className="border-t border-gray-100 hover:bg-gray-50">
                          <td className="td text-center text-gray-400">{kpi.stt}</td>
                          <td className="td text-gray-900">
                            {kpi.ten_kpi}
                            <span className="block text-xs text-gray-400 font-mono mt-0.5">{kpi.kpi_id}</span>
                          </td>
                          <td className="td text-center text-gray-500 text-xs">{kpi.don_vi}</td>
                          <td className="td px-1.5">
                            <input type="number" step="any" disabled={locked || !editMode}
                              value={form[kpi.kpi_id + '_value'] ?? ''}
                ...
                <tr className="border-t-2 border-gray-300 bg-yellow-50">
                  <td className="px-3 py-2 text-center font-bold text-gray-700">C</td>
                  <td colSpan={7} className="px-3 py-2 font-bold text-gray-700">Tổng điểm phòng (A + B)</td>
    ...
    <div className="space-y-4">
      ...
      <div className="card p-5 space-y-4">
        <div className="flex items-center gap-3 flex-wrap">
          <h3 className="font-semibold text-gray-800">Bảng ngưỡng điểm xếp loại</h3>
          {!editMode ? (
        ...
        <table className="w-full text-sm border border-gray-200 rounded-xl overflow-hidden">
          ...
          <tbody>
            {ROWS.map(row => (
              <tr key={row.loai} className="border-t border-gray-200 hover:bg-gray-50">
                <td className="td text-center">
                ...
                <td className="td text-gray-500 text-xs">{row.cond}</td>
                <td className="td text-center">
                  {row.key ? (
                    <input
                      type="number"
                      className={`input w-24 text-center text-sm ${!editMode ? 'bg-gray-50 text-gray-400 cursor-not-allowed' : ''}`}
                    ...
                    <span className="text-gray-400 text-xs italic">Điểm còn lại</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      <div className="card p-4 text-xs text-gray-500 space-y-1">
        <p className="font-medium text-gray-700">Lưu ý khi thay đổi ngưỡng</p>
...
export default function KpiInputModule() {
  ...
  if (!canEditDept(user)) {
    return (
      <div className="p-3 md:p-6">
        <h2 className="text-xl font-bold text-gray-900">Nhập liệu KPI</h2>
  ...
  if (urlTab === 'cauhinh_xeploai' && !canAdmin(user)) {
    return (
      <div className="p-3 md:p-6">
        <h2 className="text-xl font-bold text-gray-900">🏅 Cấu hình xếp loại</h2>
        <div className="mt-6"><AccessDenied /></div>
      </div>
    );
  }
  ...
  return (
    <div className="p-3 md:p-6 space-y-5">
      <div>
        <h2 className="text-lg md:text-xl font-bold text-gray-900">{TAB_TITLES_EM[urlTab] ?? 'Nhập liệu KPI'}</h2>
