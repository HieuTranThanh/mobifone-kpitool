 ...
 *
 * DỮ LIỆU ĐẦU VÀO:
 * - nv_library (localStorage ← CONFIG_NhanVien): thư viện NV
 ...
 *
 * DỮ LIỆU ĐẦU RA:
 * - nv_library → syncNvLibrary (CONFIG_NhanVien)
 * - nv_snapshot_YYYY-MM → saveNvSnapshot → syncToGas (CONFIG_Store)
 * - nhomCvLibrary → syncNhomCvLibrary; kvLibrary → syncKvLibrary
 *
 * PHÂN QUYỀN (TODO):
 * - Admin: CRUD thư viện NV, tạo/xóa tháng.
 * - Manager/Viewer: chỉ xem danh sách NV theo tháng.
 *
 * LƯU Ý:
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 flex-wrap items-center">
        <h4 className="font-semibold text-sm text-gray-800">{icon} {title} <span className="text-gray-400 font-normal">({catalog.length})</span></h4>
      ...
      {addForm && (
        <div className="flex gap-2 items-end bg-blue-50 rounded-xl p-3">
          <div>
            <label className="block text-xs text-gray-500 mb-0.5">Mã (tự động)</label>
            <input className="input text-xs w-36 bg-gray-100" readOnly value={generateId()} />
          </div>
          <div className="flex-1">
            <label className="block text-xs text-gray-500 mb-0.5">Tên *</label>
          ...
          <tbody>
            {displayed.map((item, i) => (
              <Fragment key={item[idField]}>
                <tr className={`border-t border-gray-100 hover:bg-gray-50 ${editingId === item[idField] ? 'bg-blue-50' : ''}`}>
                  ...
                  <td className="td font-medium text-gray-900">{item[nameField]}</td>
                  <td className="td text-xs hidden md:table-cell">
                    {(() => {
                      const s = formatUsedMonths(item.usedInMonths);
                      return s
                        ? <span className="whitespace-pre-line text-gray-600 leading-relaxed">{s}</span>
                  ...
                  <td className="td text-center">
                    <div className="flex items-center justify-center gap-1">
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors ${editingId === item[idField] ? 'text-blue-600 bg-blue-100' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'}`}
                      ...
                      {item.usedInMonths?.length === 0 ? (
                        ...
                        <span className="p-1.5 text-gray-300 cursor-not-allowed" title="Đang được dùng — không thể xóa">🔒</span>
                      )}
                    </div>
                  </td>
                </tr>
            ...
            {displayed.length === 0 && (
              <tr><td colSpan={5} className="td text-center text-gray-400 py-8">Chưa có mục nào</td></tr>
            )}
          </tbody>
        </table>
        </div>
    ...
    <tr className="bg-blue-50 border-t border-blue-200">
      <td colSpan={7} className="px-4 py-3">
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            <label className="block text-xs text-gray-500 mb-0.5">Nhóm công việc</label>
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-0.5">STT</label>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md mx-4 w-full space-y-4">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">Nguồn dữ liệu</label>
          ...
          {sourceType === 'copy' && (
            <div className="grid grid-cols-2 gap-3 pl-1 pt-1">
              <div>
                <label className="block text-xs text-gray-500 mb-1">Năm nguồn</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1">Tháng nguồn</label>
...
function ThuVienNV() {
  ...
  const handleImportNv = e => {
    ...
    parseImportNv(file, entries => {
      ...
      if (doUpdates) {
        const lines = toUpdate.slice(0, 10).map(u => {
          const diffs = [];
        ...
        const more = toUpdate.length > 10 ? `\n... và ${toUpdate.length - 10} NV khác` : '';
        doUpdates = window.confirm(
          `⚠️ Phát hiện ${toUpdate.length} nhân viên đã tồn tại sẽ được CẬP NHẬT:\n\n${lines}${more}\n\n` +
          `Thay đổi thông tin NV ảnh hưởng đến dữ liệu báo cáo liên quan.\n\n` +
  ...
  const handleImportNhomCv = (file, setMsg) => {
    parseImportGenericLib(file, entries => {
      ...
      let doUpdates = toUpdate.length > 0;
      if (doUpdates) {
        const lines = toUpdate.slice(0, 10).map(u => `• [${u.nhom_cv_id}] "${u.ten_cu}" → "${u.ten_moi}"`).join('\n');
        const more  = toUpdate.length > 10 ? `\n... và ${toUpdate.length - 10} nhóm khác` : '';
        doUpdates = window.confirm(
          `⚠️ Phát hiện ${toUpdate.length} nhóm công việc đã tồn tại sẽ được CẬP NHẬT tên:\n\n${lines}${more}\n\n` +
          `Thay đổi tên nhóm CV ảnh hưởng đến dữ liệu nhân viên theo tháng và báo cáo.\n\n` +
          `OK = cập nhật + thêm mới | Hủy = chỉ thêm mới, bỏ qua cập nhật.`
        );
      }
  ...
  const handleImportKv = (file, setMsg) => {
    parseImportGenericLib(file, entries => {
      ...
      let doUpdates = toUpdate.length > 0;
      if (doUpdates) {
        const lines = toUpdate.slice(0, 10).map(u => `• [${u.kv_id}] "${u.ten_cu}" → "${u.ten_moi}"`).join('\n');
        const more  = toUpdate.length > 10 ? `\n... và ${toUpdate.length - 10} khu vực khác` : '';
        doUpdates = window.confirm(
          `⚠️ Phát hiện ${toUpdate.length} khu vực đã tồn tại sẽ được CẬP NHẬT tên:\n\n${lines}${more}\n\n` +
          `Thay đổi tên khu vực ảnh hưởng đến dữ liệu nhân viên theo tháng và báo cáo.\n\n` +
          `OK = cập nhật + thêm mới | Hủy = chỉ thêm mới, bỏ qua cập nhật.`
        );
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <span className="text-xs text-gray-400">{displayed.length} NV</span>
        <div className="ml-auto flex gap-2 flex-wrap justify-end items-center">
          {importMsg && <span className="text-xs text-green-600 font-medium">{importMsg}</span>}
      ...
      {addForm && (
        <div className="card p-3 bg-blue-50 border-blue-200 flex gap-3 items-end flex-wrap">
          <div>
            <label className="block text-xs text-gray-500 mb-0.5">Mã NV</label>
          ...
          <div className="flex-1 min-w-48">
            <label className="block text-xs text-gray-500 mb-0.5">Họ tên *</label>
          ...
          <tbody>
            {displayed.map((nv, i) => (
              <Fragment key={nv.nv_id}>
                <tr className={`border-t border-gray-100 ${nv.archived ? 'opacity-60' : 'hover:bg-gray-50'} ${editingId === nv.nv_id ? 'bg-blue-50' : ''}`}>
                  <td className="td text-center text-gray-400">{i + 1}</td>
                  ...
                  <td className="td font-medium text-gray-900">
                    ...
                    <span className={`sm:hidden inline-block mt-0.5 badge text-[10px] ${nv.archived ? 'bg-gray-100 text-gray-500' : 'bg-green-100 text-green-800'}`}>
                  ...
                  <td className="td text-xs hidden md:table-cell">
                    {(() => {
                      ...
                      return s
                        ? <span className="whitespace-pre-line text-gray-600 leading-relaxed">{s}</span>
                        : <span className="text-gray-400 italic">Chưa có tháng</span>;
                    })()}
                  </td>
                  <td className="td text-center hidden sm:table-cell">
                    <span className={`badge ${nv.archived ? 'bg-gray-100 text-gray-500' : 'bg-green-100 text-green-800'}`}>
                  ...
                  <td className="td text-center">
                    <div className="flex items-center justify-center gap-1">
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors ${editingId === nv.nv_id ? 'text-blue-600 bg-blue-100' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'}`}
                        title="Chỉnh sửa">✏️</button>
                      {nv.usedInMonths.length === 0 ? (
                        <button onClick={() => handleDelete(nv)}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Xóa vĩnh viễn">🗑️</button>
                      ) : (
                        <span className="p-1.5 text-gray-300 cursor-not-allowed" title="Đang có dữ liệu — không thể xóa">🔒</span>
                      )}
                    </div>
                  </td>
                </tr>
            ...
            {displayed.length === 0 && (
              <tr><td colSpan={6} className="td text-center text-gray-400 py-8">Không có nhân viên nào</td></tr>
            )}
          </tbody>
        </table>
        </div>
        ...
        {phase === 1 ? (
          <>
            ...
            <div className="overflow-y-auto flex-1">
              <div className="bg-gray-50 px-4 py-2 flex items-center gap-2 border-b border-gray-200 sticky top-0">
                <input type="checkbox" className="w-4 h-4 rounded" checked={allChecked} onChange={toggleAll} />
                <span className="text-xs text-gray-500">Chọn tất cả ({filtered.length} NV)</span>
              </div>
              {filtered.length === 0
                ? <div className="px-4 py-6 text-sm text-gray-400 text-center">Không tìm thấy nhân viên</div>
                : filtered.map(n => (
                  <label key={n.nv_id}
                    className={`flex items-center gap-3 px-4 py-2.5 border-b border-gray-100 last:border-0 cursor-pointer ${selected.has(n.nv_id) ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                    <input type="checkbox" className="w-4 h-4 rounded"
                      checked={selected.has(n.nv_id)} onChange={() => toggle(n.nv_id)} />
                    <div>
                      <p className="text-sm font-medium text-gray-900">{n.ho_ten}</p>
          ...
          <>
            <div className="px-4 py-3 border-b flex items-center gap-3 flex-wrap">
              ...
              <span className="text-sm text-gray-600">{assignments.length} nhân viên</span>
              {assignments.length > 1 && (
                <>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-gray-500 whitespace-nowrap">Khu vực tất cả:</span>
            ...
            <div className="overflow-y-auto overflow-x-auto flex-1">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {assignments.map(a => (
                    <tr key={a.nv.nv_id} className="border-t border-gray-100">
                      <td className="td font-medium">
                        {a.nv.ho_ten}
                        <span className="block text-[10px] text-gray-400 font-mono">{a.nv.nv_id}</span>
                      </td>
                      <td className="td">
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {nvThangList.length > 0
          ...
          : <span className="text-sm text-gray-400 italic">-- Chưa có tháng --</span>}
        <span className="text-xs text-gray-400">{thang && nvThangList.length ? `${nvList.length} NV` : ''}</span>
            ...
            <table className="w-full">
              ...
              <tbody>
                {nvList.length === 0 && !showAdd && (
                  <tr><td colSpan={7} className="td text-center text-gray-400 py-8">Chưa có nhân viên nào trong tháng này</td></tr>
                )}
                {sortedNvList.map(n => [
                  <tr key={n.nv_id} className={`border-t border-gray-100 hover:bg-gray-50 ${n.archived_at ? 'opacity-60' : ''}`}>
                    <td className="td text-center text-gray-400">{n.stt}</td>
                    ...
                    <td className="td font-medium text-gray-900">
                      ...
                      <span className="block text-[10px] text-gray-500 sm:hidden">{n.khu_vuc}</span>
                    </td>
                    <td className="td text-gray-600 text-sm hidden md:table-cell">{n.nhom_cv}</td>
                    <td className="td text-gray-600 text-sm hidden sm:table-cell">{n.khu_vuc}</td>
                    <td className="td text-center hidden md:table-cell">
                      <span className={`badge whitespace-nowrap ${!n.archived_at ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
                    ...
                    <td className="td text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setEditingId(editingId === n.nv_id ? null : n.nv_id)}
                          className="p-1.5 text-gray-400 hover:text-blue-600" title="Sửa">✏️</button>
                        <button onClick={() => handleRemoveNv(n.nv_id)}
                          className="p-1.5 text-gray-400 hover:text-red-600" title="Bỏ khỏi tháng">✕</button>
                      </div>
                    </td>
                  </tr>,
                  editingId === n.nv_id && (
...
export default function DanhSachNVModule() {
  ...
  const { user } = useAuth();
  if (!canEditDept(user)) return <div className="p-3 md:p-6"><h2 className="text-xl font-bold text-gray-900">Danh sách nhân viên</h2><div className="mt-6"><AccessDenied /></div></div>;
  return (
    <div className="p-3 md:p-6 space-y-5">
      <div>
        <h2 className="text-lg md:text-xl font-bold text-gray-900">{TAB_TITLES_NV[urlTab] ?? 'Danh sách nhân viên'}</h2>
