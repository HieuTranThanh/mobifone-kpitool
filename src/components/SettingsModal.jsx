 ...
 * @description Modal cài đặt — mở cho admin và user chưa được gán role (để cấu hình kết nối ban đầu).
 *
 * CHỨC NĂNG:
 * - Tab "Kết nối": Cấu hình Supabase URL, Anon Key, Phòng ID; kiểm tra kết nối; đồng bộ thư viện.
 * - Tab "Người dùng" (chỉ admin): Xem/sửa role + display_name của user; thêm user mới vào app_users.
 *
 * PHÂN QUYỀN:
 * - Tab "Người dùng" chỉ render khi canAdmin(user); data layer thêm RLS admin_manage bảo vệ.
 */
...
import { useAuth, canAdmin, ROLE_LABELS } from '../contexts/AuthContext';
...
function ConnectionTab() {
  ...
  return (
    <div className="space-y-5">
      {/* Supabase URL */}
      <div>
        <label className="block font-medium text-sm text-slate-800 mb-1">Supabase Project URL</label>
      ...
      {/* Anon Key */}
      <div>
        <label className="block font-medium text-sm text-slate-800 mb-1">Supabase Anon Key (public)</label>
      ...
      {/* Phong ID */}
      <div>
        <label className="block font-medium text-sm text-slate-800 mb-1">
          Phòng ID (mặc định) <span className="text-slate-400 font-normal">(UUID từ bảng phong)</span>
        </label>
      ...
      {import.meta.env.DEV && (
        <div className="border border-orange-200 rounded-xl p-4 space-y-3">
          <p className="font-semibold text-sm text-slate-800">📄 Ghi thư viện vào initialData.js</p>
      ...
      {/* Hướng dẫn */}
      <details className="bg-slate-50 rounded-xl p-4 text-xs text-slate-700">
        <summary className="font-semibold cursor-pointer text-sm text-slate-800">📖 Hướng dẫn thiết lập Supabase lần đầu</summary>
        <ol className="mt-3 space-y-2 list-decimal list-inside">
          ...
          <li>Tải lại trang — ứng dụng tự đồng bộ dữ liệu thư viện từ Supabase về</li>
        </ol>
      </details>
...
function UsersTab() {
  ...
  const handleSave = async () => {
    setStatus('');
    const roleNoPhong = editData.role === 'admin' || editData.role === 'branch_viewer';
  ...
  const handleAddUser = async () => {
    ...
    const roleNoPhong = newUser.role === 'admin' || newUser.role === 'branch_viewer';
  ...
  if (loading) return <p className="text-sm text-slate-400 text-center py-8">⏳ Đang tải...</p>;
      ...
      <div className="border border-slate-200 rounded-xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
              ...
              <tr key={u.id} className="hover:bg-slate-50">
                {editId === u.id ? (
                  <>
                    ...
                    <td className="td text-xs text-slate-400 font-mono hidden sm:table-cell">{u.id.slice(0, 8)}…</td>
                    <td className="td">
                    ...
                    <td className="td hidden md:table-cell">
                      {(editData.role === 'admin' || editData.role === 'branch_viewer') ? (
                        <span className="text-xs text-slate-400 italic">Dùng PhongSwitcher</span>
                      ) : (
                    ...
                    <td className="td text-center space-x-1">
                      ...
                      <button onClick={() => setEditId(null)} className="text-xs text-slate-400 hover:text-slate-600">Hủy</button>
                    </td>
                  </>
                ) : (
                  <>
                    <td className="td font-medium text-slate-900">{u.display_name || <span className="text-slate-400">—</span>}</td>
                    <td className="td text-xs text-slate-400 font-mono hidden sm:table-cell">{u.id.slice(0, 8)}…</td>
                    <td className="td">
                    ...
                    <td className="td text-xs text-slate-500 font-mono hidden md:table-cell">{u.phong_id ? u.phong_id.slice(0, 8) + '…' : '—'}</td>
            ...
            {users.length === 0 && (
              <tr><td colSpan={5} className="td text-center text-slate-400 py-8">Chưa có user nào.</td></tr>
            )}
          </tbody>
        </table>
        </div>
      ...
      {showAdd ? (
        <div className="border border-blue-200 rounded-xl p-4 bg-blue-50 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">User ID (UUID)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Tên hiển thị</label>
            ...
            <div>
              ...
              {(newUser.role === 'admin' || newUser.role === 'branch_viewer') ? (
                <p className="text-xs text-slate-400 italic mt-1">Không cần — dùng PhongSwitcher để chọn phòng xem</p>
              ) : (
...
export default function SettingsModal({ onClose }) {
  ...
  const TABS = [
    ...
    ...(isAdmin ? [{ id: 'users', label: '👥 Người dùng' }] : []),
  ];
