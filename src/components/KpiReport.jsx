 ...
 *
 * PHÂN QUYỀN:
 * - Tất cả role đều xem được toàn bộ báo cáo và xuất Excel (chỉ đọc).
 *
 * LƯU Ý:
...
import {
  isConnected,
  getOutput as sbGetOutput,
  getDetail as sbGetDetail,
  getInputCN as sbGetInputCN,
...
const REPORT_COL_W = [40, 280, 80, 100, 100, 88, 88, 80, 100, 100];
const REPORT_TOTAL_W = REPORT_COL_W.reduce((s, w) => s + w, 0);
...
function computePhongData(thang) {
  ...
  const kq_chinhanh   = parseFloat(inp.diem_kpi_chinhanh_kq) || 0;
  const diem_chinhanh = parseFloat(inp.diem_kpi_chinhanh) || (kq_chinhanh * ty_le_chinhanh / 100);
  const tongDiem = diem_chinhanh + diemPhongSum;

  return { kpiPhong, nhomMap, kpiCalc, kq_chinhanh, diem_chinhanh, diemPhongSum, tongDiem, ty_le_chinhanh, ty_le_phong, inp };
}

// Xuất toàn bộ ra Excel
function exportAllToExcel(thang, hideZeroWeight = false) {
  ...
  const { kpiPhong, nhomMap, kpiCalc, kq_chinhanh, diem_chinhanh, diemPhongSum, tongDiem, ty_le_chinhanh, ty_le_phong } = phong;
  ...
  // === Sheet 1: KPI Phòng ===
  const rowA = Object.assign(['A', 'KPI Chi nhánh', '', kq_chinhanh > 0 ? r2(kq_chinhanh) : '', '', '', '', ty_le_chinhanh, diem_chinhanh > 0 ? r2(diem_chinhanh) : '', ty_le_chinhanh], { __tag: 'A' });
  const rowB = Object.assign(['B', 'KPI Phòng', '', '', '', '', '', ty_le_phong, diemPhongSum > 0 ? r2(diemPhongSum) : '', ty_le_phong], { __tag: 'B' });
  const phongRows = [
    Object.assign([`Báo cáo KPI Phòng — Tháng ${thang.replace('-', '/')}`], { __tag: 'title' }),
    Object.assign([`KPI Chi nhánh: ${kq_chinhanh > 0 ? parseFloat(kq_chinhanh.toFixed(2)) + '%' : '—'}   |   Điểm KPI Phòng: ${parseFloat(diemPhongSum.toFixed(2))}đ   |   Tổng: ${parseFloat(tongDiem.toFixed(2))}đ`], { __tag: 'sub' }),
    HEADER,
    rowA,
    rowB,
  ];

  const kpiPhongExp = hideZeroWeight ? kpiPhong.filter(k => { const w = kpiCalc[k.kpi_id]?.w; return w == null || w !== 0; }) : kpiPhong;
  let lastNhomId = null, stt = 0;
  kpiPhongExp.forEach(k => {
    ...
    phongRows.push([stt, k.ten_kpi, k.don_vi || '', c && c.value !== null ? r2(c.value) : '', c && c.dispPct ? fmtPctDisp(c.dispPct) : '', c && c.upper !== null ? r2(c.upper) : '', c && c.lower !== null ? r2(c.lower) : '', c && c.w !== null ? r2(c.w) : '', c && c.diem !== null ? r2(c.diem) : '', c ? c.max_pct : '']);
  });
  phongRows.push(Object.assign(['C', 'Tổng điểm phòng (A + B)', '', '', '', '', '', '', r2(tongDiem), ''], { __tag: 'C' }));
  ...
  // === Sheet 2: Tổng hợp điểm KPI NV ===
  {
    ...
    sumRows.forEach((r, i) => {
      ...
      const dataCells = [
        ...
        { v: r.nhom_cv, t: 's' },
        { v: r.khu_vuc, t: 's' },
        r.tong_diem != null ? { v: r2(r.tong_diem), t: 'n' } : { v: '', t: 's' },
  ...
  nvList.forEach(nv => {
    ...
    const cnRowA = Object.assign(['A', 'KPI Phòng', '', tongDiem > 0 ? r2(tongDiem) : '', '', '', '', ty_le_cn_phong, diemPhongDongGop > 0 ? r2(diemPhongDongGop) : '', ''], { __tag: 'A' });
    const cnRowB = Object.assign(['B', 'KPI Cá nhân', '', '', '', '', '', ty_le_cn_cn, diemCaNhanSum > 0 ? r2(diemCaNhanSum) : '', ''], { __tag: 'B' });
    const cnRows = [
    ...
    const kpiCaNhanExp = hideZeroWeight ? kpiCaNhan.filter(k => { const w = kpiCalcCN[k.kpi_id]?.w; return w == null || w !== 0; }) : kpiCaNhan;
    let lastNhomCNId = null, sttCN = 0;
    kpiCaNhanExp.forEach(k => {
      ...
      cnRows.push([sttCN, k.ten_kpi, k.don_vi || '', c && c.value !== null ? r2(c.value) : '', c && c.dispPct ? fmtPctDisp(c.dispPct) : '', c && c.upper !== null ? r2(c.upper) : '', c && c.lower !== null ? r2(c.lower) : '', c && c.w !== null ? r2(c.w) : '', c && c.diem !== null ? r2(c.diem) : '', c ? c.diemMax : '']);
    });
    cnRows.push(Object.assign(['C', 'Tổng điểm', '', '', '', '', '', '', tongDiemCN > 0 ? r2(tongDiemCN) : '', ''], { __tag: 'C' }));
...
function BaoCaoPhongTab({ thang }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {hasData && (
        <div className="flex flex-wrap gap-3 text-sm bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5">
          <span>
            <span className="text-slate-500">KPI Chi nhánh: </span>
            <strong>{kq_chinhanh > 0 ? `${parseFloat(kq_chinhanh.toFixed(2))}` : '—'}</strong>
            <span className="text-slate-400 text-xs ml-1">(→ {fmt(diem_chinhanh)} đ)</span>
          </span>
          <span className="text-slate-300">|</span>
          <span>
            <span className="text-slate-500">KPI Phòng: </span>
      ...
      <div className="card p-0 print:shadow-none print:border-0">
        <div className="text-center py-4 px-6 border-b rounded-t-xl">
          <p className="text-base font-bold uppercase tracking-wide">Bảng đánh giá kết quả thực hiện KPI Phòng</p>
          <p className="text-slate-500 mt-1 text-sm">Tháng {thang.replace('-', '/')}</p>
        </div>

        <div ref={headerScrollRef} className="sticky top-0 z-10 overflow-hidden border-b border-blue-100">
          <table className="text-xs border-collapse" style={{ tableLayout: 'fixed', width: '100%', minWidth: REPORT_TOTAL_W }}>
            <colgroup>{REPORT_COL_W.map((w, i) => <col key={i} style={{ width: `${w / REPORT_TOTAL_W * 100}%` }} />)}</colgroup>
            {THEAD}
          </table>
        </div>
          ...
          <table className="text-xs border-collapse" style={{ tableLayout: 'fixed', width: '100%', minWidth: REPORT_TOTAL_W }}>
            ...
            <tbody>
              ...
              <tr className="bg-green-50 text-green-900 font-semibold">
                ...
                <td className="px-3 py-2 border border-green-200">KPI Chi nhánh</td>
                <td className="border border-green-200" />
                <td className="px-2 py-2 text-right border border-green-200">{kq_chinhanh > 0 ? `${parseFloat(kq_chinhanh.toFixed(2))}` : '—'}</td>
                <td className="border border-green-200" />
                <td className="border border-green-200" />
                <td className="border border-green-200" />
                <td className="px-2 py-2 text-right border border-green-200">{ty_le_chinhanh}</td>
                <td className="px-2 py-2 text-right font-bold text-green-700 border border-green-200">{diem_chinhanh > 0 ? fmt(diem_chinhanh) : '—'}</td>
                <td className="px-2 py-2 text-right border border-green-200">{ty_le_chinhanh}</td>
              </tr>
              ...
              {!hasData ? (
                ...
                return (
                  <tr key={kpi.kpi_id} className="border border-slate-100 hover:bg-slate-50">
                    <td className="px-2 py-2 text-center text-slate-400 border border-slate-200">{s}</td>
                    <td className="px-3 py-2 text-slate-800 border border-slate-200">
                      {kpi.ten_kpi}
                      <span className="block text-slate-400 font-mono mt-0.5">{kpi.kpi_id}</span>
                    </td>
                    <td className="px-2 py-2 text-center text-slate-500 border border-slate-200">{kpi.don_vi}</td>
                    <td className="px-2 py-2 text-right font-medium border border-slate-200">{calc && calc.value !== null ? fmt(calc.value) : ''}</td>
                    <td className={`px-2 py-2 text-right border border-slate-200${calc?.dispPct?.error ? ' text-red-500 text-xs' : calc?.dispPct?.pct != null && calc.dispPct.pct < 1 ? ' text-red-600 font-semibold' : ''}`}>{calc && calc.dispPct ? fmtPctDisp(calc.dispPct) : ''}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{calc && calc.upper !== null ? fmt(calc.upper) : ''}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{calc && calc.lower !== null ? fmt(calc.lower) : ''}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{calc && calc.w !== null ? fmt(calc.w) : ''}</td>
                    <td className="px-2 py-2 text-right font-semibold text-blue-700 border border-slate-200">
                      {calc && calc.diem !== null ? fmt(calc.diem) : ''}
                    </td>
                    <td className="px-2 py-2 text-right border border-slate-200">{calc ? calc.max_pct : ''}</td>
                  </tr>
                );
              })}
              ...
              <tr className="bg-slate-100 text-slate-800 font-bold border-t-2 border-slate-300">
                <td className="px-2 py-3 text-center border border-slate-200">C</td>
                <td colSpan={7} className="px-3 py-3 border border-slate-200">Tổng điểm phòng (A + B)</td>
                <td className="px-2 py-3 text-right text-blue-700 border border-slate-200">{hasData ? fmt(tongDiem) : '—'}</td>
...
function exportNvAllMonthsToExcel(nvId, hoTen, xepLoaiCfg, filteredThangList, hideZeroWeight = false) {
    ...
    kpiList.forEach(kpi => {
      ...
      if (ct) {
        kpiCalc[kpi.kpi_id] = {
          ...
          pctDisp: fmtPctDisp(kpiDisplayPct(ct.value, ct.upper, kpi.upper_gt_lower, 1)),
        };
      } else {
        const parseNum = key => {
  ...
  const sumRows = allMonthData.map((d, i) => {
    ...
    return [i + 1, d.thang, hoTen,
      d.diemPhongDG != null ? r2(d.diemPhongDG) : '',
      d.diemCaNhan > 0 ? r2(d.diemCaNhan) : (d.diemRow?.diem_ca_nhan != null ? r2(d.diemRow.diem_ca_nhan) : ''),
      d.tongDiem != null ? r2(d.tongDiem) : '', xl, xepLoaiLabel(xl), d.thang, note];
  });
  ...
  allMonthData.forEach(({ thang, kpiList, kpiCalc, nhomMap, ty_le_ph, ty_le_cn, diemCaNhan, diemPhongDG, phongRaw, tongDiem, nvInfo, cnStatus }) => {
    const cnRowA = Object.assign(
      ['A', 'KPI Phòng', '', phongRaw > 0 ? r2(phongRaw) : '', '', '', '', ty_le_ph, diemPhongDG > 0 ? r2(diemPhongDG) : '', ''],
      { __tag: 'A' }
    );
    const cnRowB = Object.assign(
      ['B', 'KPI Cá nhân', '', '', '', '', '', ty_le_cn, diemCaNhan > 0 ? r2(diemCaNhan) : '', ''],
      { __tag: 'B' }
    );
    const cnRows = [
    ...
    const kpiListExp = hideZeroWeight ? kpiList.filter(k => { const w = kpiCalc[k.kpi_id]?.w; return w == null || w !== 0; }) : kpiList;
    let lastNhomId = null, stt = 0;
    kpiListExp.forEach(k => {
      if (k.nhom_id !== lastNhomId) {
        lastNhomId = k.nhom_id;
        const nhom = nhomMap[k.nhom_id];
      ...
      cnRows.push([
        ...
        c && c.value !== null ? r2(c.value) : '',
        c?.pctDisp || '',
        c && c.upper !== null ? r2(c.upper) : '',
        c && c.lower !== null ? r2(c.lower) : '',
        c && c.w !== null ? r2(c.w) : '',
        c && c.diem !== null ? r2(c.diem) : '',
        c && c.diemMax != null ? c.diemMax : '',
      ]);
    });
    cnRows.push(Object.assign(
      ['C', 'Tổng điểm', '', '', '', '', '', '', tongDiem > 0 ? r2(tongDiem) : '', ''],
      { __tag: 'C' }
    ));
...
function exportPhongAllMonthsToExcel(filteredThangList) {
  ...
  const sumRows = allMonthData.map((d, i) => {
    ...
    return [
      i + 1, d.thang,
      d.kq_chinhanh > 0 ? r2(d.kq_chinhanh) : '',
      d.diem_chinhanh > 0 ? r2(d.diem_chinhanh) : '',
      d.diemPhongSum > 0 ? r2(d.diemPhongSum) : '',
      d.tongDiem > 0 ? r2(d.tongDiem) : '',
      d.thang,
      note,
    ];
  });
  ...
  allMonthData.forEach(({ thang, kpiPhong, nhomMap, kpiCalc, kq_chinhanh, diem_chinhanh, diemPhongSum, tongDiem, ty_le_chinhanh, ty_le_phong, phStatus }) => {
    const rowA = Object.assign(
      ['A', 'KPI Chi nhánh', '', kq_chinhanh > 0 ? r2(kq_chinhanh) : '', '', '', '', ty_le_chinhanh, diem_chinhanh > 0 ? r2(diem_chinhanh) : '', ty_le_chinhanh],
      { __tag: 'A' }
    );
    const rowB = Object.assign(
      ['B', 'KPI Phòng', '', '', '', '', '', ty_le_phong, diemPhongSum > 0 ? r2(diemPhongSum) : '', ty_le_phong],
      { __tag: 'B' }
    );
    const phongRows = [
      Object.assign([`Báo cáo KPI Phòng — Tháng ${thang.replace('-', '/')}`], { __tag: 'title' }),
    ...
    phongRows.push(Object.assign(
      ['C', 'Tổng điểm phòng (A + B)', '', '', '', '', '', '', r2(tongDiem), ''],
      { __tag: 'C' }
    ));
...
// ============================================================
function BaoCaoCaNhanTab({ thang, onLoadingChange, initialNvId = '', hideZeroWeight, onHideZeroWeightChange }) {
  const snap      = thang ? getKpiSnapshot(thang) : null;
  const kpiCaNhan = useMemo(() =>
    (snap ? snap.kpiList : getKpiList()).filter(k => k.kpi_cap === 'ca_nhan').sort((a, b) => a.stt - b.stt),
    [thang, snap] // eslint-disable-line react-hooks/exhaustive-deps
  );
  ...
  const [nvId, setNvId]             = useState(() =>
    (initialNvId && nvList.some(n => n.nv_id === initialNvId)) ? initialNvId : (nvList[0]?.nv_id || '')
  );
  ...
  const handleExportAllMonths = async () => {
    ...
    exportNvAllMonthsToExcel(nvId, nv?.ho_ten || nvId, getXepLoaiConfig(), monthsForYear, hideZeroWeight);
  };

  useEffect(() => {
    if (nvList.length && !nvList.find(n => n.nv_id === nvId)) setNvId(nvList[0]?.nv_id || '');
  }, [thang, nvList]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [thang, nvId, refreshKey]); // eslint-disable-line react-hooks/exhaustive-deps

  const nvInputStatus = useMemo(() => getInputCNStatus(thang, nvId), [thang, nvId, refreshKey]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (!isConnected() || !thang || !nvId) return;
    onLoadingChange?.(true);
    Promise.all([sbGetOutput(thang), sbGetDetail(thang, nvId)])
      .then(([outRes, detRes]) => {
        if (outRes.data?.length > 0) {
          const all       = getOutputDiem();
  ...
  }, [thang, nvId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-2 no-print">
        {/* Dòng 1: chọn NV + badge trạng thái + nút xuất */}
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {showXepLoai && (
            <span className={`px-3 py-1 rounded-full text-xs font-bold border ${XEP_LOAI_CLS[showXepLoai] || ''}`}>
              ...
              {isLocalOnly && <span className="ml-1 font-normal text-slate-500">(tạm tính từ dữ liệu local)</span>}
            </span>
          )}
          {!showXepLoai && <span className="text-xs text-slate-400 italic">Chưa có đủ dữ liệu để tính điểm</span>}
      ...
      <div className="card p-0 print:shadow-none print:border-0">
        <div className="text-center py-4 px-6 border-b rounded-t-xl">
          ...
          <p className="mt-1.5 text-sm">
            ...
            {nv && <span className="text-slate-400 ml-3 text-xs">({nv.nhom_cv} · {nv.khu_vuc})</span>}
          </p>
        </div>
        ...
        <div ref={bodyScrollRef} className="overflow-x-auto" onScroll={syncFromBody}>
          <table className="text-xs border-collapse" style={{ tableLayout: 'fixed', width: '100%', minWidth: REPORT_TOTAL_W }}>
            ...
            <tbody>
              {/* Row A: KPI Phòng */}
              <tr className="bg-indigo-50 text-indigo-900 font-semibold border border-slate-200">
              ...
              {/* Row B: KPI Cá nhân */}
              <tr className="bg-blue-50 text-blue-900 font-semibold border border-slate-200">
                <td className="px-2 py-2 text-center border border-blue-200">B</td>
              ...
              {filteredTableRows.map((row, idx) => {
                ...
                const dispPctObj = ct
                  ? kpiDisplayPct(ct.value, ct.upper, kpi.upper_gt_lower, 1)
                  : lc?.dispPct;
                const pctDisp = dispPctObj ? fmtPctDisp(dispPctObj) : '';
                const pctErr  = dispPctObj?.error;
                const rawPct  = dispPctObj && 'pct' in dispPctObj ? dispPctObj.pct : null;
                ...
                return (
                  <tr key={kpi.kpi_id} className="hover:bg-slate-50 border border-slate-100">
                    <td className="px-2 py-2 text-center text-slate-400 border border-slate-200">{stt}</td>
                    <td className="px-3 py-2 text-slate-800 border border-slate-200">
                      {kpi.ten_kpi}
                      <span className="block text-slate-400 font-mono mt-0.5">{kpi.kpi_id}</span>
                    </td>
                    <td className="px-2 py-2 text-center text-slate-500 border border-slate-200">{kpi.don_vi}</td>
                    <td className="px-2 py-2 text-right font-medium border border-slate-200">{val != null ? fmt(val) : ''}</td>
                    <td className={`px-2 py-2 text-right border border-slate-200${pctErr ? ' text-red-500 text-xs' : isPctBad ? ' text-red-600 font-semibold' : ''}`}>{pctDisp}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{upper != null ? fmt(upper) : ''}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{lower != null ? fmt(lower) : ''}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{w != null ? fmt(w) : ''}</td>
                    <td className="px-2 py-2 text-right font-semibold text-blue-700 border border-slate-200">{diem != null ? fmt(diem) : ''}</td>
                    <td className="px-2 py-2 text-right border border-slate-200">{diemMax != null ? diemMax + '%' : ''}</td>
                  </tr>
                );
              })}
              ...
              <tr className="bg-slate-100 text-slate-800 font-bold border-t-2 border-slate-300">
                <td className="px-2 py-3 text-center border border-slate-200">C</td>
                <td colSpan={7} className="px-3 py-3 border border-slate-200">Tổng điểm</td>
                <td className="px-2 py-3 text-right text-blue-700 border border-slate-200">
                  {showTongDiem !== null ? fmt(showTongDiem) : '—'}
                  {isLocalOnly && showTongDiem !== null && <span className="text-slate-400 font-normal ml-1 text-xs">(tạm tính)</span>}
                </td>
                <td className="border border-slate-200" />
              </tr>
            </tbody>
...
function fmtStatCell(val, type) {
  ...
  if (type === 'pct_th') {
    if (typeof val === 'object') {
      ...
      return parseFloat((val.pct * 100).toFixed(2)) + '%';
    }
    return String(val);
  }
    ...
    if (id === 'cach_tinh') {
      ...
      for (let r = 2; r < 2 + dataRows.length; r++) {
        for (let c = 0; c < 4; c++) {
          ...
          if (c === 1 || c === 3) s.alignment = { ...s.alignment, wrapText: true, vertical: 'center' };
          ws[addr].s = s;
        }
      }
    } else {
      ...
      const dataRows = nvList.map((nv, i) => {
        ...
        return [
          ...
          ...kpiCaNhan.map(kpi => {
            ...
            if (val === null || val === undefined || val === '') return '';
            if (id === 'pct_th') {
              if (typeof val === 'object') return val.error ? val.error : r2(val.pct * 100);
            }
            const pf = parseFloat(val);
            return !isNaN(pf) ? r2(pf) : val;
          }),
        ];
      });
      ws = XLSXStyle.utils.aoa_to_sheet([
...
function ThongKeCaNhanTab({ thang, onLoadingChange }) {
  ...
  useEffect(() => {
    if (!isConnected() || !thang) return;
    onLoadingChange?.(true);
    Promise.all([sbGetInputCN(thang), sbGetDetail(thang, '')])
  ...
  const inputCNAll = useMemo(() => getInputCNByThang(thang), [thang, dataKey]); // eslint-disable-line react-hooks/exhaustive-deps
  const chiTietFallback = useMemo(() => {
    ...
    return map;
  }, [thang, dataKey]); // eslint-disable-line react-hooks/exhaustive-deps

  if (!thang) return null;

  // Chiều rộng cố định từng cột (px) — dùng cho hai-bảng sticky-header
  const COL_STT = 36, COL_HOTEN = 140, COL_NHOMCV = 100, COL_KHUVUC = 90, COL_KPI = 88;
  const colWidths = [COL_STT, COL_HOTEN, COL_NHOMCV, COL_KHUVUC, ...kpiCaNhan.map(() => COL_KPI)];
  const totalWidth = colWidths.reduce((s, w) => s + w, 0);
  // Khi body cuộn ngang, sync header để giữ header đồng bộ
  const syncFromBody = () => {
    if (headerScrollRef.current) headerScrollRef.current.scrollLeft = bodyScrollRef.current?.scrollLeft ?? 0;
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="card p-0 print:shadow-none print:border-0">
        <div className="px-4 py-2.5 border-b bg-slate-50 rounded-t-xl flex items-center justify-between">
          <span className="font-semibold text-slate-700 text-sm">
            {STAT_TABLES.find(t => t.id === tableType)?.label} — Tháng {thang.replace('-', '/')}
          </span>
          <span className="text-xs text-slate-400">{nvList.length} nhân viên · {kpiCaNhan.length} KPI</span>
        </div>
        ...
        {!kpiCaNhan.length ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              ...
              <tbody>
                {kpiCaNhan.map((kpi, i) => (
                  <tr key={kpi.kpi_id} className={`border-t border-slate-100 ${i % 2 === 0 ? '' : 'bg-slate-50'}`}>
                    <td className="px-2 py-2 text-center text-slate-500 border border-slate-200">{kpi.stt}</td>
                    <td className="px-2 py-2 font-medium text-slate-900 border border-slate-200">{kpi.ten_kpi}</td>
                    <td className="px-2 py-2 text-slate-600 border border-slate-200">{kpi.don_vi || '—'}</td>
                    <td className="px-2 py-2 text-slate-700 border border-slate-200 whitespace-pre-wrap leading-relaxed">
                      {kpi.cach_tinh || <span className="text-slate-300 italic">Chưa có mô tả</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <>
            {/* Header freeze: sticky top-0 khi cuộn dọc bằng thanh cuộn chính.
                overflow-hidden clip ngang; scrollLeft được sync từ body qua JS.
                Không có overflow trên card cha → sticky tham chiếu <main>. */}
            <div ref={headerScrollRef} className="sticky top-0 z-10 overflow-hidden border-b border-blue-100">
              <table className="text-xs border-collapse" style={{ tableLayout: 'fixed', width: '100%', minWidth: totalWidth }}>
                <colgroup>{colWidths.map((w, i) => <col key={i} style={{ width: `${w / totalWidth * 100}%` }} />)}</colgroup>
                <thead className="bg-blue-50">
                  <tr className="text-slate-600 text-xs uppercase tracking-wide">
                    <th className="px-2 py-2 text-center border border-blue-100 sticky left-0 z-[2] bg-blue-50">STT</th>
                    <th className="px-2 py-2 text-left border border-blue-100 border-r-2 border-r-slate-300 sticky z-[2] bg-blue-50" style={{ left: COL_STT }}>Họ và tên</th>
                    <th className="px-2 py-2 text-left border border-blue-100 bg-blue-50">Nhóm CV</th>
                    <th className="px-2 py-2 text-left border border-blue-100 bg-blue-50">Khu vực</th>
                    {kpiCaNhan.map(kpi => (
                      <th key={kpi.kpi_id} className="px-2 py-2 text-center border border-blue-100 bg-blue-50">
            ...
            {/* Body: cuộn ngang, sync header scrollLeft qua onScroll */}
            <div ref={bodyScrollRef} className="overflow-x-auto" onScroll={syncFromBody}>
              <table className="text-xs border-collapse" style={{ tableLayout: 'fixed', width: '100%', minWidth: totalWidth }}>
                <colgroup>{colWidths.map((w, i) => <col key={i} style={{ width: `${w / totalWidth * 100}%` }} />)}</colgroup>
                <tbody>
                  {nvList.map((nv, i) => {
                    ...
                    return (
                      <tr key={nv.nv_id} className={`group border-t border-slate-100 ${rowBg} hover:bg-blue-50/80`}>
                        <td className={`px-2 py-1.5 text-center text-slate-400 border border-slate-200 sticky left-0 z-[1] ${rowBg} group-hover:bg-blue-50/80`}>{i + 1}</td>
                        <td className={`px-2 py-1.5 font-medium text-slate-900 border border-slate-200 border-r-2 border-r-slate-300 sticky z-[1] ${rowBg} group-hover:bg-blue-50/80`} style={{ left: COL_STT }}>{nv.ho_ten}</td>
                        <td className="px-2 py-1.5 text-slate-600 border border-slate-200">{nv.nhom_cv}</td>
                        <td className="px-2 py-1.5 text-slate-600 border border-slate-200">{nv.khu_vuc}</td>
                        {kpiCaNhan.map(kpi => {
                          ...
                          return (
                            <td key={kpi.kpi_id}
                              className={`px-2 py-1.5 text-center border border-slate-200 ${
                  ...
                  {nvList.length === 0 && (
                    <tr>
                      <td colSpan={4 + kpiCaNhan.length} className="px-4 py-8 text-center text-slate-400 italic">
...
export default function KpiReport() {
  ...
  const initialNvId  = searchParams.get('nv')    || '';
  const initialThang = searchParams.get('thang') || '';

  const [caNhanSubTab, setCaNhanSubTab] = useState(() => initialNvId ? 'chi_tiet' : 'thong_ke');
  ...
  const [thang, setThang] = useState(() =>
    initialThang && thangList.includes(initialThang) ? initialThang : defaultThang(thangList)
  );

  // Đến từ Dashboard "Xem chi tiết" — đã áp dụng nv/thang vào state, xóa query param khỏi URL
  useEffect(() => {
    if (searchParams.has('nv') || searchParams.has('thang')) {
      setSearchParams({}, { replace: true });
      // <main> (Layout.jsx) giữ nguyên scrollTop cũ từ trang trước — cuộn lên đầu trang chi tiết
  ...
  const handleExportDetail = async () => {
    ...
    try {
      if (isConnected()) {
        try {
          const cnRes = await sbGetInputCN(thang);
          if (cnRes.data?.length > 0) {
            cnRes.data.forEach(row => upsertInputCN({ ...row, thang }));
          }
        } catch (_) {}
      }
      exportAllToExcel(thang, hideZeroWeight);
    } finally {
  ...
  if (!thangList.length) {
    return (
      <div className="p-3 md:p-6 text-center py-16 text-slate-400">
  ...
  return (
    <div className="p-3 md:p-6 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-3 no-print">
        <div>
          <h2 className="text-lg md:text-xl font-bold text-slate-900">{TAB_TITLES_BC[urlTab] ?? 'Báo cáo kết quả KPI'}</h2>
          <p className="text-slate-500 text-xs mt-0.5">{TAB_DESC_BC[urlTab] ?? 'Kết quả đánh giá KPI Phòng và cá nhân theo từng tháng'}</p>
        </div>
      ...
      {urlTab === 'canhan' && (
        <div className="space-y-3">
          ...
          <div className="flex gap-1 bg-slate-100 rounded-xl p-1 w-fit no-print">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  caNhanSubTab === st.id ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                {st.label}
              </button>
          ...
          {caNhanSubTab === 'chi_tiet' && <BaoCaoCaNhanTab  thang={thang} onLoadingChange={setCaNhanLoading} initialNvId={initialNvId} hideZeroWeight={hideZeroWeight} onHideZeroWeightChange={setHideZeroWeight} />}
        </div>
      )}
    </div>
  );
