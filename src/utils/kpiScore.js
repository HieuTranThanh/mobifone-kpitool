 ...
 * - kpiPercent(): hàm nội bộ dùng bởi calcNhanVien (tính pct_th); không export.
 * - Xếp loại NV cuối cùng do calcService quyết định qua xepLoaiWithConfig(ngưỡng cấu hình).
 */
...
function calcWeights(activeKpis, totalPoints = 70) {
...
export function xepLoaiColor(loai) {
  ...
  return map[loai] || 'bg-slate-100 text-slate-800';
}
