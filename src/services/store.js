 ...
 * Mọi hàm save* phải đi kèm syncToGas/syncKpiLibrary/syncNvLibrary tương ứng.
 *
 * DỮ LIỆU QUẢN LÝ:
...
export function syncToGas(key, value) {
...
export function deleteFromGas(key) {
  syncToGas(key, null);
}
...
export function resolveKpiRefs(refs, library) {
...
export function resolveNhomRefs(refs, library) {
...
// Trả về raw refs (cho những nơi cần lưu/gửi lên GAS)
...
// ── NV Library (mirrors CONFIG_NhanVien) ──────────────────────
...
export function resolveNvRefs(refs, library) {
...
export function saveKpiSnapshot(thang, kpiListOrRefs, nhomListOrRefs) {
  ...
  syncToGas(`kpi_snapshot_${thang}`, obj);
}
...
export function deleteKpiSnapshot(thang) {
  ...
  deleteFromGas(`kpi_snapshot_${thang}`);
}
...
function _readYearLock(type, thang) {
  ...
  if (localStorage.getItem(oldKey) !== null) {
    ...
    syncToGas(yearKey, yearData);
    localStorage.removeItem(oldKey);
    deleteFromGas(oldKey);
    return true;
  }
...
export function saveTrongSoConfig(thang, cfg)  { save(`trong_so_thang_${thang}`, cfg); syncToGas(`trong_so_thang_${thang}`, cfg); }
export function deleteTrongSoConfig(thang)     {
  ...
  deleteFromGas(`trong_so_thang_${thang}`);
...
export function saveXepLoaiConfig(config) {
  ...
  syncToGas('xep_loai_config', config);
}

// ── Ghi chú tháng ─────────────────────────────────────────────
export function getMonthNote(thang) {
  try { return JSON.parse(localStorage.getItem(`month_note_${thang}`)) || ''; } catch { return ''; }
}
export function saveMonthNote(thang, note) {
  if (note) {
    localStorage.setItem(`month_note_${thang}`, JSON.stringify(note));
    syncToGas(`month_note_${thang}`, note);
  } else {
    localStorage.removeItem(`month_note_${thang}`);
    deleteFromGas(`month_note_${thang}`);
  }
}
...
// ── Backup / Restore ──────────────────────────────────────────
const DYNAMIC_PREFIXES = ['kpi_snapshot_', 'trong_so_thang_', 'locked_cn_', 'locked_phong_', 'nv_snapshot_', 'input_phong_', 'trong_so_weights_', 'month_note_', 'output_meta_'];

export function exportAllData() {
  const staticKeys = [...Object.values(KEYS)];
  const dynamicKeys = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (k && DYNAMIC_PREFIXES.some(p => k.startsWith(p))) dynamicKeys.push(k);
  }
  const data = { _version: 2, _exported_at: new Date().toISOString() };
  [...staticKeys, ...dynamicKeys].forEach(k => {
...
// Các key không đưa vào bulkSyncStore khi import backup (local-only hoặc computed)
const LOCAL_ONLY_KEYS = new Set([
  'trong_so',
  'input_cn',
  'output_diem', 'output_chitiet',
]);

export async function importAllData(data) {
  const { _version, _exported_at, ...entries } = data;
  Object.entries(entries).forEach(([k, v]) => localStorage.setItem(k, JSON.stringify(v)));

  const { isConnected, syncKpiLibrary, syncNhomLibrary, syncNvLibrary, syncNhomCvLibrary, syncKvLibrary, bulkSyncStore } = await import('./supabaseService');
  if (!isConnected()) return;

  const kpiLibrary    = entries[KEYS.kpiLibrary];
  const nhomLibrary   = entries[KEYS.nhomLibrary];
  const nvLibrary     = entries[KEYS.nvLibrary];
  const nhomCvLib     = entries[KEYS.nhomCvLibrary];
  const kvLib         = entries[KEYS.kvLibrary];
  const DEDICATED_KEYS = new Set([KEYS.kpiLibrary, KEYS.nhomLibrary, KEYS.nvLibrary, KEYS.nvList, KEYS.nhomCvLibrary, KEYS.kvLibrary]);
  const storeEntries = {};
  Object.entries(entries).forEach(([k, v]) => {
    if (!DEDICATED_KEYS.has(k) && !LOCAL_ONLY_KEYS.has(k)) storeEntries[k] = v;
  });

  const promises = [];
  if (kpiLibrary?.length)  promises.push(syncKpiLibrary(kpiLibrary).catch(e => console.warn('[import kpiLib]', e.message)));
  if (nhomLibrary?.length) promises.push(syncNhomLibrary(nhomLibrary).catch(e => console.warn('[import nhomLib]', e.message)));
  if (nvLibrary?.length)   promises.push(syncNvLibrary(nvLibrary).catch(e => console.warn('[import nvLib]', e.message)));
  if (nhomCvLib?.length)   promises.push(syncNhomCvLibrary(nhomCvLib).catch(e => console.warn('[import nhomCvLib]', e.message)));
  if (kvLib?.length)       promises.push(syncKvLibrary(kvLib).catch(e => console.warn('[import kvLib]', e.message)));
  if (Object.keys(storeEntries).length)
    promises.push(bulkSyncStore(storeEntries).catch(e => console.warn('[import bulk]', e.message)));
...
export function resetToDefault() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key && DYNAMIC_PREFIXES.some(p => key.startsWith(p))) toRemove.push(key);
    // Xóa cả key cũ per-month nếu còn
    else if (key && (key.startsWith('locked_input_cn_') || key.startsWith('locked_input_phong_')))
      toRemove.push(key);
  }
