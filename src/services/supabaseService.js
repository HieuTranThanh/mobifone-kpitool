...
// ── Config accessors (thay getGasUrl/setGasUrl) ──────────────────
...
// ── getAll ────────────────────────────────────────────────────────
// Tải toàn bộ data khi app khởi động (thay GAS getAllData)
...
export async function bulkSyncStore(entries) {
  const sb = requireClient(); const phongId = requirePhong();
  const toUpsert = []; const toDelete = [];
  Object.entries(entries).forEach(([key, value]) => {
    if (value === null) toDelete.push(key);
    else toUpsert.push({ phong_id: phongId, key, value, updated_at: new Date().toISOString() });
  });
  await Promise.all([
    toUpsert.length > 0
      ? sb.from('config_store').upsert(toUpsert, { onConflict: 'phong_id,key' }).then(({ error }) => chk(error, 'bulkSync:upsert'))
      : Promise.resolve(),
    toDelete.length > 0
      ? sb.from('config_store').delete().eq('phong_id', phongId).in('key', toDelete).then(({ error }) => chk(error, 'bulkSync:delete'))
      : Promise.resolve(),
  ]);
...
export async function updateInputCNNvs(thang, addedNvs, removedNvIds, _orderedNvIds) {
  ...
  if (removedNvIds?.length > 0) {
    await Promise.all([
      sb.from('input_cn').delete().eq('phong_id', phongId).eq('thang', thang).in('nv_id', removedNvIds),
      sb.from('input_cn_nv').delete().eq('phong_id', phongId).eq('thang', thang).in('nv_id', removedNvIds),
    ]);
  }
...
export async function deleteMonthSheet(thang) {
  const sb = requireClient(); const phongId = requirePhong();
  await Promise.all([
    sb.from('input_cn').delete().eq('phong_id', phongId).eq('thang', thang),
    sb.from('input_cn_nv').delete().eq('phong_id', phongId).eq('thang', thang),
    sb.from('output_diem').delete().eq('phong_id', phongId).eq('thang', thang),
    sb.from('output_chitiet').delete().eq('phong_id', phongId).eq('thang', thang),
  ]);
...
// ── Bundle query (thay getDiemThang GAS) ─────────────────────────
